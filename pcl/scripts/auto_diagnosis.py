...
parser.add_argument('plot_amps', dest='plot_amps', action='store_true')
parser.add_argument('plot_ant_metrics', dest='plot_ant_metrics', action='store_true')
...
for ant in range(uv.Nants_telescope):
    ...
    vals[ant] = np.median(np.abs(d))
if args.plot_ant_metrics:
    vals = np.zeros(uv.Nants_telescope)
    # load ant_metrics
    am = ant_metrics.load_antenna_metrics(args.file)
    for ant in range(uv.Nants_telescope):
