...
pixelDecs = np.asarray([-30, -35, -20, -10, 5, -30, -50])/180.0*np.pi
pixelRAs = np.asarray([90, 60, 120, 76, 91, 44, 50])/180.0*np.pi
fluxes = np.asarray([100, 49, 125, 60, 20, 99, 22])

#pixelDecs = np.asarray([-30, -35, -20])/180.0*np.pi
#pixelRAs = np.asarray([90, 60, 120])/180.0*np.pi
#fluxes = np.asarray([100, 49, 125])

# pixelDecs = np.asarray([-35])/180.0*np.pi
# pixelRAs = np.asarray([60])/180.0*np.pi
# fluxes = np.asarray([49])

#spectralIndices = np.asarray([-2.45, -2.3, -2.7])
#spectralIndices = np.ones(len(fluxes)) * -2.55 + 2.0
spectralIndices = np.asarray([-2.45, -2.3, -2.7, -2.4, -2.55, -2.63, -2.51]) + 2.0

for source in sourceFiles:
    #outfilename = './SimulatedVisibilities/' + source.split('/')[-1].replace('zen','simple_noBeam')
    #outfilename = './SimulatedVisibilities/' + source.split('/')[-1].replace('zen','simple_VariedSpIndex')
    #outfilename = './SimulatedVisibilities/' + source.split('/')[-1].replace('zen','simple_BuggyBeam')
    #outfilename = './SimulatedVisibilities/' + source.split('/')[-1].replace('zen','simple_OneSource_noBeam')
    #outfilename = './SimulatedVisibilities/' + source.split('/')[-1].replace('zen','simple_SevenSources_VariedSpIndex')
#    outfilename = './SimulatedVisibilities/' + source.split('/')[-1].replace('zen','simple_SevenSources_VariedSpIndex_noBeam')
    outfilename = './SimulatedVisibilities/' + source.split('/')[-1].replace('zen','simple_SevenSources_VariedSpIndex_achromaticBeam')
    #outfilename = './SimulatedVisibilities/' + source.split('/')[-1].replace('zen','simple_SevenSources_VariedSpIndex_noBeam_integeru')
    #outfilename = './SimulatedVisibilities/' + source.split('/')[-1].replace('zen','simple_SevenSources')
    #outfilename = './SimulatedVisibilities/' + source.split('/')[-1].replace('zen','simple_3Sources')
    #outfilename = './SimulatedVisibilities/' + source.split('/')[-1].replace('zen','simple_3Sources_noBeam')
    #outfilename = './SimulatedVisibilities/' + source.split('/')[-1].replace('zen','simple_SevenSources_noBeam')
    #outfilename = './SimulatedVisibilities/' + source.split('/')[-1].replace('zen','simple')
