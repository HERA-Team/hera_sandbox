...
def runAll(dataFiles, flaggedChannels, findFlaggedChannels=False, importedBetas=None, importedSigmas = None, importedDs = None):
    ...
    #    noiseCovDiag = np.ones(noiseCovDiag.shape)
        if importedBetas is not None and importedSigmas is not None and importedDs is not None:
            print '    Ignoring logcal output and using imported Betas, Sigmas, and Ds...'
            betas, Sigmas, Ds = importedBetas.copy(), importedSigmas.copy(), importedDs.copy()
        else:
    ...
    return betas, Sigmas, Ds, flaggedChannels
...
betas0, Sigmas0, Ds0, flaggedChannels = runAll(dataFiles, [], findFlaggedChannels=True)

dataFiles = ["./Data/" + file for file in os.listdir("./Data") if file.endswith("uvcRRE.npz")]
betasFull, SigmasFull, DsFull, dummy = runAll(dataFiles, flaggedChannels)

dataFiles = ["./Data/" + file for file in os.listdir("./Data") if file.endswith("2456943.43835.xx.uvcRRE.npz")] 
betas, Sigmas, Ds, dummy = runAll(dataFiles, flaggedChannels, importedBetas=betasFull, importedSigmas = SigmasFull, importedDs = DsFull)
...
plt.plot(allChans, np.abs(betas0),'-')
plt.plot(allChans, np.abs(betasFull),'.-')
plt.plot(allChans, np.abs(betas),'.')
plt.legend(['Single Data File','All Data Files','Single File Initialzed\nWith Integrated Result'])
plt.xlabel('abs(beta)')
plt.ylabel('chan')
