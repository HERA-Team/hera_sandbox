...
allDataFiles = ["./Data/" + file for file in os.listdir("./Data") if file.endswith("uvcRRE.npz")]
for dataFile in allDataFiles:
    dataFiles = [dataFile]
    if dataFiles[-1].replace('.npz','.uCal.npz').replace('./Data/','') in os.listdir("./Data"):
        print dataFiles, 'already done.'
    else:
        try:
            pol='xx'
