...
try:
    ...
    LINE1 = None
    def ddr_filter(d, dw, drw, plot=False):
        ...
        return fft_ddr_dat / (d.shape[0] * d.shape[1])
    ...
    for iter in xrange(opts.maxiter):
        ...
        if mode == 0: # BEAMFORM
            for k in simdat:
                ...
                d_bkup = d.copy()
                print '   ', k
                d = ddr_filter(d, dw, drw, plot=True)
                #dalt = ddr_filter(d-d_bkup, dw, drw, plot=True)
        ...
        for bl in msrdat:
            ...
            for k in simdat:
                ...
                if mode == 0:
                    if _srcest_bm.has_key(k):
                        _resdat[bl] -= _srcest_bm[k] * simdat[k][bl]
                        #gi = n.sqrt(_srcest_bm[k])
                        #gj = gi.copy()
                #elif srcest_bm.has_key(k):
                #    _resdat[bl] -= srcest_bm[k] * simdat[k][bl]
                    #gi = n.sqrt(srcest_bm[k])
                    #gj = gi.copy()
                #if mode == 1:
