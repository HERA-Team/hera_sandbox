import time, logging
from PDB import StillDB
dbi = PDB('psa128',verbose=True)
...
def file_to_still(f, nstills):
    ...
    # XXX depends on neighbors, (ie  the observation's requirements as well as any requirements of other files)
    return
...
class Action:
    ...
    def has_prerequisites(self, dbi):
        ...
        return True  #XXX This is the only place I see neighbor blocking. need scheduling of neighbors as well -dcj
    def launch(self):
        '''Run this task.'''
...
class Scheduler:
    ...
    def start(self, dbi):
        ...
        while True: # XXX if threading, this should be a self.quit flag
            ...
            self.clean_completed_actions(dbi) #also clean up error states
    ...
    def get_new_active_files(self, dbi):
        ...
        #to ensure ordering
        for f in dbi.ordered_files(): #XXX presumably ordered_files is the list of files yet to be processed... -dcj
            if not dbi.is_completed(f) and not self._active_file_dict.has_key(f):
                    self._active_file_dict[f] = len(self.active_files)
                    self.active_files.append(f)  #XXX where do neighboring files get added?
    ...
    def get_action(self, dbi, f):
        ...
        next_step = FILE_PROCESSING_STAGES[status]# check that it is not required for a neighbor
        ...
        a = Action(f, next_step, neighbors, still) #XXX the neighbors must be scheduled first.
        if a.has_prerequisites(dbi): return a  #XXX Just waits on neighbors.  They are not scheduled.
        else: return None
