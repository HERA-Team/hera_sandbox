...
"""
Generate a joint set or union of two catalogs.

cat_intersection.py -s all --cat=catA,catB
...
#setup logging
if opts.vverb: logging.disable(logging.DEBUG);print '!'
...
print '#',"found",len(catA),"in",catA_name
print '#',"found",len(catB),"in",catB_name
update_pos(catA)
catA.update_jys(opts.freq)
update_pos(catB)
catB.update_jys(opts.freq)
print "#"+' '.join(sys.argv)
print "# Name=" +catA_name + "\t NameB=" + catB_name 
if opts.vo:
    print "#Name \t Ra \t Dec \t S_nu_ \t e_S_nu_ \t NameB \t Altnames \t Seq"
else:
     print "#Name \t Ra \t Dec \t S_nu_ \t e_S_nu_ \t NameB "
...
#    print "%s \t %s \t"%(src.ra,src.dec),
    ...
    if opts.vo:
        ...
        if len(vodata)<2: 
            print "%s \t %s \t %s \t %07.3f \t %07.3f \t %s"%\
                (name,src.ra,src.dec,src.jys,err,fsrc.src_name)
        ...
        print "%s \t %s \t %s \t %07.3f \t %07.3f \t %s \t %s \t %d"%\
            (name,src.ra,src.dec,src.jys,err,fsrc.src_name,altnames,int(seq))
    else:
        print "%s \t %s \t %s \t %07.3f \t %07.3f \t %s"%\
            (name,src.ra,src.dec,src.jys,err,fsrc.src_name)
print "# %d sources found in the joint set"%(count)