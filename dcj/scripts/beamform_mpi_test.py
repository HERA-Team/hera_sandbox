...
o.add_option('--dt',type='str',default='1n',
    help='Amount of time to step at a time. Give number with units (n or s eg number or seconds). default 1n')
...
#                start_prms[obj][prm] = prms[obj][prm][0]
    ...
    start_prms.update(cat.get_params(gsm_dict))
    ...
    for file in args:
        ...
        for p,d in uv.all():
            ...
            if (tnext == t or rnum==nrec):
                ...
                if not 'helm' in catalogs:
                    print "computing an initial beamform"
                    print "starting flux comp"
                    
                    if opts.time: tbmform = time.time()
                    fluxes = beamform(srcs) #grid operation.
                    if opts.time: timelog(timefile,'beamform',time.time()-tbmform)

                    #Mash up the results for logging
                    #fit for a power law
                    jys = []
                    fits = []
                    for flux in fluxes:
                        fit = n.polyfit(n.log10(aa.get_afreqs()),n.ma.log10(n.ma.clip(flux,0.001,1e8)),1,full=True)
                        fit,res = fit[0],fit[1]
                        fits.append((fit[0],flux[0],n.sqrt(res[0]/len(flux))))
                    fits = n.array(fits)     
                    
                    #select sources to keep
                    #keep those with rms flux within 1std deviation
                    rms_std = n.std(fits[:,2])
                    rms_av = n.average(fits[:,2])
                    good_srcs = [srcs[i] for i in range(len(srcs)) if rms_std+rms_av>n.abs(fits[i,2])\
                        and (rms_av-rms_std)<n.abs(fits[i,2])]
                    bad_srcs = {}
                    bad_cat = {}
                    for i,src in enumerate(srcs):
                        print src,cat[src].mfreq,
                        print fits[i,1]*(cat[src].mfreq/aa.get_afreqs()[0])**cat[src].index,fits[i,0],fits[i,2],
                        if src in good_srcs:
                            print
                            jys.append(fits[i,1]*(cat[src].mfreq/aa.get_afreqs()[0])**cat[src].index)
                            cur_prms[src]['jys'] = fits[i,1]*(cat[src].mfreq/aa.get_afreqs()[0])**cat[src].index
                        else:
                            print 'NA*'
#                        start_prms[src]['index'] = fits[i,0]
                    prm_list, key_list = a.fit.flatten_prms(cur_prms)
                    prm_list = n.array(prm_list)
                    
                    #update the nodes with the new catalog
                    comm.bcast(4,root=0)
                    comm.bcast(good_srcs,root=0)
                    srclog(good_srcs)

                    print "Starting with beamformed fluxes"
                    a.fit.print_params(cur_prms)
                else:
                ...
                #generate catalog parameters for logging
                bmprms = cat.get_params() 
                for i,src in enumerate(good_srcs):
                    bmprms[src].update({'jys':fits[i,1],'index':fits[i,0],'e_S_nu':fits[i,2],'mfreq':aa.get_afreqs()[0]})
                for i,src in enumerate([src for src in srcs if not src in good_srcs]): #add empty errors to the missing sources
                    bmprms[src]['e_S_nu'] = 0    
                bmlog(bmprms,tnow,lstnow)  #LOG EVENT
                
                print "Beam formed fluxes"
                bmfluxes = dict(zip(srcs,fluxes[:,0]))
                for s in srclist:
                    if not s in srcs: bmfluxes[s] = -1
                for name,flux0,flux_avg in zip(good_srcs,fits[:,1],fits[:,0]):
                    print '     %s = %10.3f,%10.3f'%(name,flux0,flux_avg)
                for name in [src for src in cat if src not in good_srcs]:
                    print '     %s (not up)'%(name)
                print '------------------------------------------------------------'
                tnext += dt
                lstnow=uv['lst']
