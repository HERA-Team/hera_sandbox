...
for infile in args:
    ...
    freq = proj.sub((proj.specaxnum)).toworld(n.array([0]))[0] #grab the 
    try:
    ...
    for srcname,src in ccat.iteritems():
        ...
        pRA,pDec = MAP.toworld(n.argwhere(image==Flux).squeeze())
        ...
        TYPE = dtype([('RA','<f8',(1)),
               ...
               ('catflux','<f8'),
               ('freq','<f8')])
        results[srcname].append(d.view(TYPE))
if opts.outfile is None: outfile = '.'.join(infile.split('.')[:-1]+['vot'])
else: outfile = opts.outfile
names = results.keys()


nsrcs= n.max([len(results[name]) for name in names]) #the largest number of sources we have
myblanksrc = n.zeros(len(d)).view(TYPE)
print nsrcs
for name in names:
    #shape up the list of measurements for each source into a dict of arrays that all have the same shape
    fillblanks = True
    while fillblanks: 
        if len(results[name])<nsrcs:results[name].append(myblanksrc.copy())
        else: fillblanks=False
    mydata = n.vstack(results[name])
    results[name] = mydata

#save the data points in a vot file    
D = n.vstack([results[name]['srcpx'].squeeze() for name in names])
t.add_column('srcpx',D,
