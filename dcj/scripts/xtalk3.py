...
o.add_option('--dn',default=1,type=int,
    help='number of files over which to smooth time dependent gain for reprocess step[1]')
...
if opts.reprocess:
    ...
    if opts.dn>1:
        for bl in xsum: gain[bl] = smooth(n.array(gain[bl]),window_len=opts.dn)
