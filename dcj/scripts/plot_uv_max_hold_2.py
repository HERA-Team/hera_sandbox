...
o.add_option('--hold_type',dest='hold',default='max',
   help='Max, min, avg, [Max]')
...
for uvfile in args:
    ...
    for (uvw,t,(i,j)),d in uv.all():
        ...
        if not plot_x_max.has_key(bl): plot_x_max[bl] = []
        if opts.hold=='max':
            if first_run==0: 
                max = d
                first_run = 1 
            d = n.abs(d)
            max = n.where(d > max, d, max)
            plot_x[bl].append(d)
            plot_x_max[bl].append(max)
    del(uv)

#alternate plotter experiment
bls = plot_x.keys()
...
for cnt, bl in enumerate(bls):
    ...
    d_max = n.ma.concatenate(plot_x_max[bl], axis=0)
...
#        is_chan_range = False
    if opts.mode.startswith('phs'): d = n.angle(d.filled(0))
        ...
        d = n.ma.masked_less_equal(d, 0)
        d = n.ma.log10(d)
        d_max = n.ma.absolute(d_max.filled(0))
        d_max = n.ma.masked_less_equal(d_max, 0)
        d_max = n.ma.log10(d_max)
    ...
    if is_chan_range and is_time_range:
        if opts.fringe:
            ...
            if opts.time_axis == 'index':
               ...
               t1,t2 = plot_t['jd'][0], plot_t['jd'][-1]
                #t1,t2 = jd2hrs(plot_t['jd'][0]),jd2hrs(plot_t['jd'][-1])
        ...
        if not opts.hold is None: 
            print "entering experimental plot mode" 
            p.imshow(d, extent=(c1,c2,t2,t1), aspect='auto', 
                vmax=dmax, vmin=dmin, cmap=p.get_cmap('gray_r'),
                interpolation='nearest')
            p.rcParams['contour.negative_linestyle']='solid'
            p.colorbar()
            if opts.time_axis.startswith('physical'):
                d_max = n.flipud(d_max)
            p.contour(d_max,[-0.5],linewidth=0.5,linestyles='solid',colors='r',aspect='auto',
               hold=1,alpha=0.5,extent=(c1,c2,t2,t1))
        else: 
            p.imshow(d, extent=(c1,c2,t2,t1), aspect='auto', 
