...
import aipy as a, numpy as n, sys, optparse, ephem, os,string
...
o.add_option('--fmt', dest='fmt', default='%s_%04d',
    help='A format string for counting successive images written to files.  Default is %s_%04d (i.e. <src>_<snapcnt> or cyg_0045).')
...
o.add_option('--facet_dec',default='-90_90',
    help='Range of DECs to include in facets.')
o.add_option('--facet_cnt',default=None,type=int,
   help='Specify the a facet number to end processing. Process from --skip to --facet_cnt.')
o.add_option('--print_only',action='store_true',
    help='Just print action but output no data [experimental]')
o.add_option('--use_head',action='store_true',
   help='Use the header to skip files with lsts below altmin. Increases faceting speed')
o.add_option('--use_facet_num',action='store_true',
   help='Use the source count variable instead of the name of a source in the file name.')

opts, args = o.parse_args(sys.argv[1:])

declim = n.array(map(float,opts.facet_dec.split('_')))*n.pi/180
if opts.facet_cnt is None: opts.facet_cnt = opts.facets
def btwn(ab,x):
    return ab[0]<=x and x<ab[1]
...
if opts.src == 'zen':
    ...
    srcs = [a.phs.RadioFixedBody(ra,dec,name=str(i)) 
        for i,(ra,dec) in enumerate(zip(ras,decs)) if btwn(declim,dec)]
    for ra in ras:
        print repr(dec),declim,btwn(declim,dec)
    cat = a.phs.SrcCatalog(srcs)
    for src in cat:
        cat[src].src_name = pos2name(cat[src]._ra,cat[src]._dec)
...
def fname(ftag, src_name,cnt): return '%s.%s.fits' % (opts.fmt % (src_name,cnt), ftag)
def to_fits(ftag,i,src,src_name,cnt,history=''):
    filename = fname(ftag,src_name,cnt)
...
for srccnt, s in enumerate(cat.values()):
    ...
    if srccnt > opts.facet_cnt: continue
    s.compute(aa)
    if opts.use_facet_num: srcname = "%04d"%(srccnt+1)
    else: srcname = s.src_name
    ...
    for filename in args:
      ...
      #if source is far from lst of file, continue
      if opts.use_head:
          jd = float(string.join(filename.split('.')[1:3],'.'))
          aa.set_jultime(jd)
          s.compute(aa)
          if n.abs(s.ra-uv['lst'])>(90 - opts.altmin)*a.img.deg2rad: 
              continue
      print "generating images: " 
      for k in outputs: print '\t', fname(k, srcname,0)
      print "for pointing: ", s.ra,s.dec
      print '-'*15
      if opts.print_only: 
          continue
      # Read all data from each file
      for (crd,t,(i,j)),d,f in uv.all(raw=True):
          f = n.logical_or(f,n.isnan(d))
#          d.mask = n.logical_or(d.mask,n.isnan(d))
          d[n.argwhere(n.isnan(d))] = 0
          history = uv['history']
                  ...
                  if snapcnt == 0:
                      if curtime != None:
                          ...
                          for k in ['uvs','bms','dim','dbm']:
                              if k in outputs: to_fits(k, eval(k), s, srcname,snapcnt,history=history)
                          imgcnt += 1
                      us,vs,ws,ds,wgts = [],[],[],[],[]
                      im = Img(opts.size, opts.res, mf_order=0)
                      if opts.src == 'zen':
    ...
    try:
        ...
        print 'No data for %s; skipping output file.'%(srcname)
        continue
    for k in ['uvs','bms','dim','dbm']:
        if k in outputs: to_fits(k, eval(k), s, srcname, 0,history=history)
    imgcnt += 1
    us,vs,ws,ds,wgts = [],[],[],[],[]
    im = Img(opts.size, opts.res, mf_order=0)
