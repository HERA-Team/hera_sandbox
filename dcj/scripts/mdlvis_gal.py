...
"""
import numpy as n, aipy as a, optparse, os, sys, ephem,glob
...
if len(args) > 0:
    ...
    freqs = a.cal.get_freqs(opts.sdf, opts.sfreq, opts.nchan)
...
if not opts.map is None:
    ...
    print "found ",len(px)," active pixels"
    try: mflx, i_poly = h[px]
    except(ValueError):
        mflx = h[px]
        i_poly = [n.zeros_like(mflx)]
    mind = i_poly[0]    # Only implementing first index term for now
    fscale = n.array([(f/opts.freq)**ind for f,ind in zip(freqs,mind)])
    mflx = n.outer(mflx,fscale)
    mmfq = opts.freq * n.ones_like(mind)
#    mmfq = n.array([opts.freq])
...
def est_dims():
    """
    Estimate the number of operations to be undertaken in a "scratch" simulation.
    """
    #source count
    if opts.src: 
        nstuff=len(cat)
    ...
    if opts.map:
        nstuff *= len(mflx)
    nstuff *= len(aa)*(len(aa)+1)/2.0
    if opts.sim:
        nstuff *= opts.nchan*(opts.endjd-opts.startjd)/opts.inttime*3600.0*24
    else:
        nstuff *= uvi['nchan']*(uvi['endjd']-uvi['startjd'])/uv['inttime']*3600.0*24
    return nstuff
...
#        print "flx.shape,mfq.shape = ",flx.shape,mfq.shape
...
if len(args) > 0:
    ...
    for filename in args:
        ...
        print "creating file:", filename, ", degrees of freedom:", est_dims()
    ...
    pols = opts.pol.split(',')
    root_date = opts.startjd
    files = glob.glob('*.'+str(root_date)+'_*')
    if len(files)!=0:
        indices = n.array([int(f.split('_')[1].split('.')[0]) for f in files])
        maxind = n.max(indices)
        filename = 'zen.'+str(root_date)+'_'+str(maxind+1)+'.uv'
    else:
        filename = 'zen.'+str(root_date)+'_0.uv'
    print "creating file:", filename, ", degrees of freedom:", est_dims()    
    ...
    for cnt,t in enumerate(times):
        print 'Timestep %d / %d. LST: %s' % (cnt+1, len(times),str(aa.sidereal_time()))
        aa.set_jultime(t)
