...
bls = n.array(bls)
if rank==0: 
    print "Modelling %i baselines on %i nodes (%f bls/node)"%(len(bls),size,len(bls)/size)
    print "Modelling a total of %i times for a total of %i data points"%(len(times),len(bls)*len(times)*no_data.size)
#bl_times = []
#for t in times:
#    for bl in bls:
#        bl_times.append((bl[0],bl[1],t))
bl_local = comm.scatter([None]+n.array_split(bls,size-1))
curtime = 0
...
if rank != 0:
    for t in times:
        for N,bl in enumerate(bl_local):
    ...
    maxint = 100
    for tcnt,t in enumerate(times):
        datas = comm.gather([])
        print "cnt: %i, t: %f"%(tcnt,t)
        data = n.concatenate(datas[1:])
#        del(datas)
        if tcnt%maxint==0:
            if tcnt>0:print "deleting uv"; del(uv)
            filename = '.'.join(('sim',"%12.5f"%t,'uv'))
            print rank," creating: "+filename
        ...
        for p,d,f in data:
            ...
            uv['lst'] = p['lst']
            uv['ra'] = p['lst']
            if tcnt>90: print preamble, len(d),len(f)
