...
for filename in args:
    ...
    eta = n.fft.fftfreq(len(freqs),d=n.diff(freqs)[0])
...
print 'freqs = ',pspec_sum.keys()
print 'bins = '
bincounts = []
bin_nums = []
for f in bins:
    for bin in bins[f]:
        bincounts.append(len(bins[f][bin]))
bin_nums += bins[f].keys()
        

print "samples/bin summary"
print "min = ",n.min(bincounts)," median = ",n.median(bincounts)," max = ",n.max(bincounts)
print "number of unique bins = ",len(list(set(bin_nums)))
#print 'bins = ',pspec_sum[pspec_sum.keys()[0]].keys()
pkumag_sum,pkumag_wgt = {},{}
for fq in pspec_sum:
    pkumag_sum[fq],pkumag_wgt[fq] = {},{}
    for bin in pspec_sum[fq]:
        if len(bins[fq][bin])<2: 
            bins[fq].pop(bin)
            continue
