...
o.add_option('-w',action='store_true',
    help='Weight by dynamic range of image.')
...
for i, filename in enumerate(args):
    ...
    img = img.squeeze()
    img_wgts.append(n.abs(img).max()-n.abs(img).min())
    ...
    if opts.w:
        skymap.add((ex,ey,ez), map_wgts*img_wgts[i], img)
    else:
        skymap.add((ex,ey,ez), map_wgts, img)
if opts.w:
    skymap.map.map /= n.sum(img_wgts)
