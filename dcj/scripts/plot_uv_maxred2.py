...
a.scripting.add_standard_options(o, pol=True, chan=True, dec=True,
...
o.add_option('-g', dest='g', type='int',default=1,
    help='Number of grid units to plot. Can be 1 to (number of columns)-1. [1]')
...
grid_num_inv = dict(zip(grid_num.values(),grid_num.keys()))
...
a.scripting.uv_selector(uv, pol_str=opts.pol)
...
#Generate the selection string for the desired grid spacing
ants = ''
rows = ['A','B','C','D']
ncols = 8
for row in rows:
    for col in range(ncols-opts.g):
        ants += "%d_%d"%(grid_num[str(row)+str(col)],grid_num[str(row)+str(col+opts.g)])
	ants += ','
print "ants = ",
print ants
...
for uvfile in args:
    ...
    #print opts.pol
    a.scripting.uv_selector(uv, ants, pol_str=opts.pol)
    # Read data from a single UV file
    for (uvw,t,(i,j)),d in uv.all():
        #bl = '%d,%d' % (i,j)
        bl = '%s%s'%(grid_num_inv[i],grid_num_inv[j][1])
        if opts.correct and grid_num_inv[j]>grid_num_inv[i]: d = n.conjugate(d)
...
for cnt, bl in enumerate(bls):
    ...
    if ((m2-1)*m1)>cnt: p.xticks([]);p.xlabel('')
    if not opts.share: p.text(0.2,0.8,bl,transform=ax.transAxes)#p.title(bl)
    #if not opts.share: p.title(bl)
if is_chan_range and is_time_range:p.colorbar()
