...
N1 =a.healpix.HealpixMap(nside=64) 
N2 =a.healpix.HealpixMap(nside=64) 
#h = a.healpix.HealpixMap(nside=32)
Sky.map = n.random.normal(scale=2,size=Sky.map.size)
N1.map = n.random.normal(size=N1.map.size)
N2.map = n.random.normal(size=N2.map.size)
Sky.set_interpol(True)
N1.set_interpol(True)
N2.set_interpol(True)
...
for ha in n.arange(-n.pi/16, n.pi/16, 2*n.pi / (24*60)):
    ...
    Tsky1 = Sky[ex,ey,ez] +N1[ex,ey,ez]
    Tsky1.shape = resp.shape
    img1 = Tsky1 * resp
    img1 = a.img.recenter(img1, (img1.shape[0]/2, img1.shape[1]/2))
    uv1 = n.fft.fft2(img1)

    Tsky2 = Sky[ex,ey,ez] +N2[ex,ey,ez]
    Tsky2.shape = resp.shape
    img2 = Tsky2 * resp
    img2 = a.img.recenter(img2, (img2.shape[0]/2, img2.shape[1]/2))
    uv2 = n.fft.fft2(img2)


#    UV = lambda U,V: interp2d(urange,urange,n.real(uv)) + interp2d(urange,urange,n.imag(uv))*1j
    
    plt_uv = a.img.recenter(n.abs(uv1), n.array(uv1.shape)/2)[SZ:SZ+64,SZ:SZ+64]
...
#        Sd.append(uv[ui-1:ui+2,vj-1:vj+2])
        Smult.append(uv1[ui,vj].squeeze()*n.conjugate(uv2[ui,vj].squeeze()))
        Sadd.append(uv1[ui,vj].squeeze())
        

    Mr = n.poly1d(n.polyfit(F,n.real(Smult),1))
    Mi = n.poly1d(n.polyfit(F,n.real(Smult),1))
    Spmult_all.append(Mr(F)+Mi(F)*1j)
    Mr = n.poly1d(n.polyfit(F,n.real(Sadd),1))
    Mi = n.poly1d(n.polyfit(F,n.real(Sadd),1))
    Spadd_all.append(Mr(F)+Mi(F)*1j)

kpar = n.linspace(eta2kparr(1/(B*1e9),f212z(fo*1e9)),eta2kparr(len(F)/(B*1e9)/2.,f212z(fo*1e9)),num=len(F)/2.)
