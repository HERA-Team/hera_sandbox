...
#o.add_option('-s', '--scale', dest='scale', type='float', default=122500000.,
#    help='An additional numerical scaling to apply to the data.  Default:  
#12250000.')
...
for filename in args:
    ...
    if os.path.exists(maskfile) and opts.apply_masknpz:
        ...
        mask = n.sum(n.array([m for m in mask.values()]),axis=0)
        for bl in f:                 
            f[bl] |= mask
    for bl in d:
        i,j = bl2ij(bl)
    ...
    n.savez(filename+'_cal0.npz',C=C,C_gain=C_gain)
    #if True:
