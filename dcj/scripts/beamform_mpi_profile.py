...
Does the usual bit, but for only one timestamp.
"""
...
def bmlog(prms,jd,lst,extra=[]):
    pflat,kys = a.fit.flatten_prms(prms)
    if not os.path.exists(root+'/bm_keys.pkl'):
...
#                data = n.concatenate((data,rec))                
            if (tnext == t or rnum==nrec):
                ...
                if not 'helm' in catalogs:
                    ...
                    #mash up the results for display, and to hand to the fitter.
...
#                        start_prms[src]['jys'] = jys[i]
                    ...
                    for i,src in enumerate(srcs):
                        ...
                        start_prms[src]['jys'] = fits[i,1]*(cat[src].mfreq/aa.get_afreqs()[0])**cat[src].index
#                        start_prms[src]['index'] = fits[i,0]
                ...
                for i,src in enumerate(srcs):
                    bmprms[src]={'jys':fits[i,1],'index':fits[i,0],'e_S_nu':fits[i,2],'mfreq':aa.get_afreqs()[0]}
                ...
                for name,flux0,flux_avg in zip(srcs,fits[:,1],fits[:,0]):
