...
for cnt, ch in enumerate(chs_list):
    ...
    fov = None
    for pfx in chs[ch]:
        dima, kwds = a.img.from_fits(pfx+'0002.dim.fits')
        dimb, kwds = a.img.from_fits(pfx+'0002b.dim.fits')
        dimr, kwds = a.img.from_fits(pfx.replace('tsys_3day','rs_tsys_3day') \
                +'0001.dim.fits')
        dbma, kwds = a.img.from_fits(pfx+'0002.dbm.fits')
        dbmb, kwds = a.img.from_fits(pfx+'0002b.dbm.fits')
        ...
        dim1,dim2 = dimr / 2., (dima - dimb) / 2
        dbm, dbm_debug = (dbma + dbmb) / 2., (dbma - dbmb) / 2
        ...
        #p.subplot(121)
        #p.imshow(n.log10(bmsq[-1]))
        #p.subplot(122)
        #p.imshow(n.log10(uv1sq[-1]))
        #p.show()
    ...
    for rpx in rpxs:
        ...
        if wgt1 == 0: continue
        wgt2 = (bmsq_r**2).sum()
        ps1.append(uv1sq_r.sum() / wgt1)
        ps2.append(uv2sq_r.sum() / wgt1)
        # Estimate "cosmic variance" around ring by removing average
    ...
    ps1 = n.array(ps1); ps2 = n.array(ps2)
...
p.ylabel(r'$C(|u|) \left[K^2\right]$', fontsize=20)
p.show()
