...
for i,sep in enumerate(plotseps):
    ...
    for bl in strbls[sep].split(','):
        if len(args)==2:
            ...
            if opts.d:
                ...
                except KeyError: print 'KeyError',bl
                delays = n.fft.fftfreq(len(W),d=n.diff(P['freqs'])[0])
                plot(n.fft.fftshift(delays),n.fft.fftshift(n.abs(_d+info['res'])))
                
                xarr.append(n.fft.fftshift(delays))
                medarr.append(n.fft.fftshift(n.abs(_d+info['res'])))
                
                ylim([0,MAX])
            else:
                try: 
                	plot(P['freqs']*1e3,P[bl],label=bl)
                	xarr.append(P['freqs']*1e3)
                	medarr.append(P[bl])
                #plot->semilogy X
                #P[bl] -> n.abs(P[bl]) X
                except KeyError: print 'baseline KeyError',bl
                ylim([-MAX,MAX])
    ...
    if not opts.cal is None:
        ...
        if opts.d: vlines([-bl_length,bl_length],0,MAX,lw=2,linestyles='dashed')
mytitle = filename
if opts.d:mytitle += ' delay'
#fig.canvas.set_window_title(mytitle)
show()

medarr=n.array(medarr)
xarr=n.array(xarr)

plot(np.median(xarr,axis=0),np.median(medarr,axis=0),'b-',lw=2)
if not opts.cal is None and opts.d:
	for k,(i,j) in enumerate([(41,49),(49,47),(41,29),(58,13)]):
		uvw = aa.get_baseline(i,j)
		bl_length = n.sqrt(n.dot(uvw,uvw))
		vlines([-bl_length,bl_length],0,MAX,linestyles='dashed',alpha=1./float(k+1))
ylabel('|V|')
if opts.d: xlabel('delay (ns)')
else: xlabel('MHz')
show()
close()
