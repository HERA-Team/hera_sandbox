...
for arg in args:
    ...
    for iter in xrange(opts.maxiter):
        ...
        if iter < BURN_IN:
            step['burn'] = True
            dw, drw = int(float(opts.dw+1)*iter*NTIERS/BURN_IN) % (opts.dw+1), int(float(opts.drw+1)*iter*NTIERS/BURN_IN) % (opts.drw+1)
            tier = NTIERS * iter / BURN_IN
        ...
        for k in simdat:
            ...
            if not k in srctier[tier]:
                ...
                #    except(KeyError): pass
                continue
            else: print '    GAIN: %s %f' % (k, _clean_gain[k])
            #if not steps[-1]['srcs'].has_key(k):
            if not step['srcs'].has_key(k):
                d, w = 0, 0
                for bl in simdat[k]:
        ...
        print '    Prob:', n.exp((steps[-1]['score'] - step['score'])/EXP_NOISE)
        if iter == 0 or n.random.uniform() < n.exp((steps[-1]['score'] - step['score'])/EXP_NOISE):
        #if True:
            ...
            steps.append(step)
            resdat = _resdat
            clean_gain = _clean_gain
            # Draw plots
            if iter > BURN_IN and iter % 2 == 0:
                ...
                if LINE1 is None:
                    ...
                    for pltnum,k in enumerate(coeffs):
                        _c = n.average(n.average([n.abs(from_coeffs(steps[-1]['srcs'][k][i]))**2 for i in steps[-1]['srcs'][k]], axis=0), axis=1)
                        P.subplot(232)
                    ...
                    for k in coeffs:
                        ...
                        #LINE[k].set_ydata(n.average(n.abs(from_coeffs(_c))**2, axis=1))
                        LINE[k].set_ydata(_c)
                        LINE2[k].set_data(h2[k])
        else:
            for k in clean_gain:
                if not k in srctier[tier] or clean_gain[k] < .05: continue
                clean_gain[k] = max(.75 * clean_gain[k], .01)
