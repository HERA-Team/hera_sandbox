...
def fit_func(prms, filelist, decimate, decphs):
    ...
    for t in dbuf:
        ...
        for bl in dbuf[t]:
            ...
            difsq = difsq.sum()
            score += difsq
            score_by_bl[bl] = score_by_bl.get(bl,0.) + difsq
            nsamples_by_bl[bl] = nsamples_by_bl.get(bl,0.) + nsamp
            nsamples += nsamp
    ...
    if first_fit is None:
        first_fit = score
        for bl in score_by_bl:
            blscore = n.sqrt(score_by_bl[bl] / nsamples_by_bl[bl])
            if blscore > .7:
                bad_bls.append(bl)
                print a.miriad.bl2ij(bl), blscore
        print 'Omitting %d/%d bad baselines' % (len(bad_bls), len(score_by_bl))
        score, nsamples = 0.,0.
        for bl in score_by_bl:
            if bl in bad_bls: continue
            score += score_by_bl[bl]
            nsamples += nsamples_by_bl[bl]
        score = n.sqrt(score / nsamples)
        first_fit = score
