...
for srccnt, s in enumerate(cat.values()):
    ...
    for filename in args:
      ...
      for (crd,t,(i,j)),d,f in uv.all(raw=True):
          ...
          # Bail if we don't have enough data to do a good line fit
          if f.sum() > f.size/2: continue
          ...
          d = d.compress(valid)
          # Remove a linear freq component in real/imag
          import pylab
          pylab.plot(d.real, 'r.')
          pylab.plot(d.imag, 'b.')
          if True:
              _x = n.arange(d.size)
              poly_r = n.polyfit(_x, d.real, deg=1)
              poly_i = n.polyfit(_x, d.imag, deg=1)
              sm_d = n.polyval(poly_r, _x) + 1j*n.polyval(poly_i, _x)
              pylab.plot(sm_d.real, 'r:')
              pylab.plot(sm_d.imag, 'b:')
              d -= sm_d
              pylab.plot(d.real, 'r-')
              pylab.plot(d.imag, 'b-')
          else:
