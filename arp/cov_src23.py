...
for arg in args:
    ...
    for iter in xrange(opts.maxiter):
        ...
        step = {'srcs':{}, 'd_ant':{}}
        if iter < BURN_IN: step['burn'] = True
        else: step['burn'] = False
        if iter == 0: # BEAMFORM
            dw, drw = 0, 0
            ...
            if iter < BURN_IN:
                ...
                mask = 1
            _clean_gain = max(n.random.uniform(.75,1.25) * clean_gain, .01)
            print '    DW=%d DRW=%d' % (dw, drw),
            print '_CLEAN_GAIN=%f (CLEAN_GAIN=%f)' % (_clean_gain, clean_gain)
        ...
        if iter == 0 or n.random.uniform() < n.exp((steps[-1]['score'] - step['score'])/EXP_NOISE):
            ...
            if iter > BURN_IN and iter % 2 == 0:
                ...
                for k in ['cyg','cas'] + plot_srcs[:2]:
                    coeffs[k] = n.array([s['srcs'][k][i] for s in steps[1:] for i in s['srcs'][k] if not s['burn']])
                    _c = coeffs[k].flatten()
                    _c_abs = n.abs(_c)
            ...
            clean_gain = max(.75 * clean_gain, .01)
