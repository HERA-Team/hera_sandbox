...
#bl_sets = bl_sets[:7] # just do one set of redundant bls
#bl_sets = bl_sets[7:] # just do one set of redundant bls
...
print 'Bandwidth:', n.around(B, 4), 'GHz @', n.average(fqs[CH0:CH1])
...
overall_sum, overall_wgt = 0, 0
overall_fg_sum, overall_fg_wgt = 0, 0
for bltype,bls in enumerate(bl_sets):
    color = colors[bltype % len(colors)]
    pspec_sum[bltype] = 0; pspec_wgt[bltype] = 0
    pspec_fg_sum[bltype] = 0; pspec_fg_wgt[bltype] = 0
    cnt, bl0 = 0, bls[0]
    ...
    # Fit delay of first baseline assuming symmetry around 0
    ...
    for bl in bls:
        ...
        if False:
            small_window = a.dsp.gen_window(CH1-CH0, window='kaiser3')
            gain = n.average(small_window*vs[CH0:CH1])
            dly[bl] = n.fft.ifft(small_window*ds[CH0:CH1])
            dly_fg[bl] = n.fft.ifft(small_window*n.fft.fft(c_d)[CH0:CH1])
        elif False:
            gain = n.average(vs[CH0:CH1])
            dly[bl] = n.fft.ifft(ds[CH0:CH1])
            dly_fg[bl] = n.fft.ifft(n.fft.fft(c_d)[CH0:CH1])
        else:
            gain = n.average(vs[CH0:CH1])
            width = CH1-CH0
            dly[bl] = C.pfb.pfb(ds[CH0-width:CH1+width], window='kaiser3', taps=3, fft=n.fft.ifft)
            dly_fg[bl] = C.pfb.pfb(n.fft.fft(c_d)[CH0-width:CH1+width], window='kaiser3', taps=3, fft=n.fft.ifft)
        ...
        if PLOT:
            P.subplot(221)
            #P.plot(fqs, n.abs(ds/n.where(vs>0,vs,1)), color+',', alpha=0.2)
            #P.plot(fqs, vs, 'k-')
            ...
            #P.subplot(223)
            #taus = n.fft.fftfreq(c_d.size, d=fqs[1]-fqs[0])
            #P.semilogy(n.abs(taus), n.abs(c_d), color+',', alpha=0.2)
    
    # Sum up power spectrum
    for cnt,bl0 in enumerate(bls):
        for bl1 in bls[cnt+1:]:
            pspec_sum[bltype] += dly[bl0] * n.conj(dly[bl1]) * jy2T**2
            pspec_fg_sum[bltype] += dly_fg[bl0] * n.conj(dly_fg[bl1]) * jy2T**2
            pspec_wgt[bltype] += wgt[bl0] * wgt[bl1]
            pspec_fg_wgt[bltype] += wgt_fg[bl0] * wgt_fg[bl1]

    if PLOT:
        P.subplot(223)
        plt_dly = n.sqrt(n.abs(pspec_sum[bltype] / pspec_wgt[bltype]))
        plt_dly_fg = n.sqrt(n.abs(pspec_fg_sum[bltype] / pspec_fg_wgt[bltype]))
        ...
        print 'ks:', ks[:3]
        # The residual pspec
        pspec_fold_sum = pspec_sum[bltype][:ks.size/2]
        pspec_fold_sum[1:] += pspec_sum[bltype][:ks.size/2:-1]
        pspec_fold_sum[0] *= 2
        pspec_fold_wgt = 2*pspec_wgt[bltype]
        overall_sum += pspec_fold_sum; overall_wgt += pspec_fold_wgt
        pspec_fold = pspec_fold_sum / pspec_fold_wgt
        kbins, pspec_fold = C.pspec.rebin_log(ks[:ks.size/2], pspec_fold, nbins=10)
        print 'kbins:', kbins[:3]
        pspec_fold = n.abs(pspec_fold)
        # The foreground pspec
        pspec_fold_fg_sum = pspec_fg_sum[bltype][:ks.size/2]
        pspec_fold_fg_sum[1:] += pspec_fg_sum[bltype][:ks.size/2:-1]
        pspec_fold_fg_sum[0] *= 2
        pspec_fold_fg_wgt = 2*pspec_fg_wgt[bltype]
        overall_fg_sum += pspec_fold_fg_sum; overall_fg_wgt += pspec_fold_fg_wgt
        pspec_fg_fold = pspec_fold_fg_sum / pspec_fold_fg_wgt
        junk, pspec_fg_fold = C.pspec.rebin_log(ks[:ks.size/2], pspec_fg_fold, nbins=10)
        ...
        P.loglog(kbins, 1e3*pspec_fold, color+':^')
        P.loglog(kbins, 1e3*pspec_fg_fold, color+':v')
...
pspec_fold = overall_sum / overall_wgt
kbins, pspec_fold = C.pspec.rebin_log(ks[:ks.size/2], pspec_fold, nbins=10)
pspec_fold = n.abs(pspec_fold)
#P.subplot(224)
#cosmo_scalar = X2Y * kbins**3 / (2*n.pi**2) * Omega * B
...
P.subplot(221); P.xlim(fqs[0], fqs[-1]); P.ylim(-1000,1000)
P.subplot(222); P.xlim(fqs[0], fqs[-1]); P.ylim(-n.pi, n.pi)
P.subplot(223); P.ylim(1e-1,1e3); P.grid()
P.subplot(224); P.ylim(1e1,1e11); P.grid()
