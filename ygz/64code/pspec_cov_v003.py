...
days = dsets1.keys()
for k in days:
    lsts1[k],data1[k],flgs1[k] = get_data(dsets1[k], antstr=antstr, polstr=POL, rmbls=rmbls, verbose=True)
...
for k in days:
    ...
    # print lsts2[k].shape
    for bl in data1[k]:
        data1[k][bl],flgs1[k][bl] = n.array(data1[k][bl][i1:i1+j]),n.array(flgs1[k][bl][i1:i1+j])
    for bl in data2[k]:
...
#         if PLOT and True:
#             #p.plot(S); p.show()
#             p.subplot(311); capo.arp.waterfall(x1[k][bl], mode='real')
#             p.subplot(323); capo.arp.waterfall(C[k][bl])
#             p.subplot(324); p.plot(n.einsum('ij,jk',n.diag(S),V).T.real)
#             p.subplot(313); capo.arp.waterfall(_Cx1[k][bl], mode='real')
