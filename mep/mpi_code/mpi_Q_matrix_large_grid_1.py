...
beamsig = 10.*n.pi/180.
savekey = 'grid_5_ns_gauss_10_deg_run_{0}_'.format(run_num)
fq = 0.1
...
baselines = baselines[run_num*252:((run_num+1)*252-1),:]
...
if rank==master:
    ...
    for kk in range(numToDo):
        ...
        savebuffer[save_count] = n.array([selectedi,selectedj,entry])
        save_count += 1
        ...
        # if 200 elements have been calculated, write out to file
        if save_count==2:
            n.savez_compressed('./Q_matrices/backup/{0}Q_backup_{1}'.format(savekey,num_saved),Q=savebuffer)
            print "Master completed save number ", num_saved
            save_count = 0
            num_saved +=1
            savebuffer = n.zeros([200,3])
