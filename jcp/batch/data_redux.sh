...
for FILE in $UVCR; do
    ...
    if ! ls ${FILE}R &> /dev/null; then
        ...
        /usr/global/paper/bin/xrfi_simple.py -a 1 --combine -t 20 -n 4 ${FILE}E --to_npz=${FILE}E.npz
        /usr/global/paper/bin/xrfi_simple.py -a all --combine -t 20 ${FILE} --from_npz=${FILE}E.npz
    fi
    if ls ${FILE}R &> /dev/null; then
