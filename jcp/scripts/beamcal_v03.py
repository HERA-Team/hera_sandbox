...
srclist0 += srcs
srclist1 = []
srclist1.extend(srclist0)
for i in xrange(3):
     print i
     random.shuffle(srclist0)
     srclist1.extend(srclist0)


fluxes,gains,wgts = {},{},{}
for k in srclist1:
    fluxes[k] = srcfluxes[k]/offset
    gains[k] = srcgains[k]/offset
    ...
    if n.any(crossing):
        ...
        print k, beamgain
        f = n.sum((gains[k][crossing]*beam.wgt[x[k][crossing],y[k][crossing],z[k][crossing]])/((beam.map[x[k][crossing],y[k][crossing],z[k][crossing]])/beam.wgt[x[k][crossing],y[k][crossing],z[k][crossing]]))/n.sum(beam.wgt[x[k][crossing],y[k][crossing],z[k][crossing]])
        print f, n.max(gains[k])
        gains[k] /= f
    ...
    beam.add((x[k],y[k],z[k]), wgts[k], gains[k])
    if False: #k == 'cyg':
    ...
    if True:
        ...
        beam.add((xx,yy,zz),wgts[k],gains[k])
