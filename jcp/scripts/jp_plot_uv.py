...
"""
Currently tests parts of my attemp to butcher plot_uv.py
"""
...
#JP's pitifully added options
o.add_option('--sub',dest='subfile',help='Subtract the equivalent plot from another file')
...
#JP added this nonsense
if opts.subfile: args.append(opts.subfile)
...
for uvfile in args:
    ...
    for (uvw,t,(i,j)),d in uv.all():
        ...
        d = d.take(chans)
        d.shape = (1,) + d.shape
        #More Jonnie nonsense
        if not plot_x.has_key(bl): plot_x[bl] = []
        if not sub_x.has_key(bl): sub_x[bl] = []
        if uvfile == opts.subfile: sub_x[bl].append(d)
        else: plot_x[bl].append(d)
...
for cnt, bl in enumerate(bls):
    ...
    if opts.subfile:
        s = n.ma.concatenate(sub_x[bl], axis=0)
        #d = n.ma.column_stack(d,s)
    ...
    if opts.mode.startswith('phs'): d = n.angle(d.filled(0))
        ...
        d = n.ma.log10(d)
        if opts.subfile:
            s = n.ma.absolute(s.filled(0))
            s = n.ma.masked_less_equal(s, 0)
            s = n.ma.log10(s)
    ...
    if is_chan_range and is_time_range:
        ...
        if opts.subfile:
            p.imshow(d/s, extent=(c1,c2,t2,t1), aspect='auto', 
