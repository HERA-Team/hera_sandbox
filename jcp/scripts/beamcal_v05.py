...
for k in srcs:
    ...
    track[k] = n.append(n.unique(beama.crd2px(x[k],y[k],z[k])), n.unique(beama.crd2px(-x[k],-y[k],z[k])))
...
for k in srcs:
    for i,meas in enumerate(srcfluxes[k]):
        pcrd = beama.crd2px(n.array([x[k][i]]),n.array([y[k][i]]),n.array([z[k][i]]))
        ncrd = beama.crd2px(n.array([-x[k][i]]),n.array([-y[k][i]]),n.array([z[k][i]]))
