...
for uvfile in args:
    ...
    def res_mfunc(uv, p, d, f):
        ...
        #define bin to image: lots of HARDCODING here
        k_img = .30
        dly_img = k_img / C.pspec.dk_deta(C.pspec.f2z(.1525))
        bin_dly = 1. / (sdf * nchan)
        bin_img = dly_img/bin_dly
        uthresh, lthresh = bin_img + 1.5, -bin_img - 0.5
        uthresh, lthresh = int(n.ceil(uthresh)), int(n.floor(lthresh))
        area = n.ones(_d.size, dtype=n.int)
        area[uthresh:lthresh] = 0
        #compare with baseline length + 50ns
        bl_len = n.sqrt(n.dot(aa.get_baseline(i,j),aa.get_baseline(i,j)))
        #print dly_img, i,j, bl_len
        if dly_img > bl_len + 50: area[uthresh+2:lthresh-2] = 1
        else: return p, None, n.ones_like(d)
