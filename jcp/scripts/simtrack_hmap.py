...
o.add_option('-b','--beam',dest='beam',default='/data3/paper/pober/pgb/pgb16/pgb015/gbeam0.5x0.5y.fits',
    help='The beam hmap file to use.')
...
for f in args:
    ...
    beam = a.map.Map(fromfits=opts.beam)

    track,mask = [],[]
    for ix,iy,iz in zip(x,y,z):
        if iz > 0:
            track.append((beam[ix,iy,iz])*cat[k].jys)
            mask.append(0)
        else:
            mask.append(1)
    if not opts.sim:
        afreqs = npz['afreqs']
        outfile = f.replace('f.npz','sG.npz') #% int(opts.noise))
    else:
        afreqs = (n.arange(100.,200,(100./1024)))/1000.
        #print afreqs
        outfile = k+'__sG.npz' #% int(opts.noise)
    if opts.noise != 0.:
        print 'adding noise'
        track = n.array(track)
