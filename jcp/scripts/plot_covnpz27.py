...
for cnt, k in enumerate(srcs):
    d,w = 0.,0.
    for bl in srcdata[k]:
        if bl != a.miriad.ij2bl(3,3): continue
        d += srcdata[k][bl]
    ...
    for jd in t:
        aa.set_jultime(jd)
        day = n.floor(jd)
        if not lsts.has_key(day): lsts[day] = []
        lsts[day].append(aa.sidereal_time())
    ...
    for day in lsts:
        print cnt
        p.semilogy(lsts[day], vs_time[cnt:cnt+len(lsts[day])], ',', label=day)
        cnt += len(lsts[day])
