...
for arg in args:
    ...
    nfiles = 3
    #print float(files[0].split('.')[2])
    #print float(files[1].split('.')[2])
    #what follows is a super cheap hack
    if float(files[0].split('.')[2]) < float(files[1].split('.')[2]) - 10000.:
        files.pop(0)
        nfiles = 2
        print 'POP!!!!!'
    ...
    #ntimes = len(times)/nfiles
    #times = times[ntimes:2*ntimes]
    #t_balun = t_balun[ntimes:2*ntimes]
    #t_cable = t_cable[ntimes:2*ntimes]
    #n.savez('%s__info.npz' % (arg), times=n.array(times), freqs=afreqs, scores=scores, t_balun=n.array(t_balun), t_cable=n.array(t_cable))
