...
a.scripting.add_standard_options(o, cal=True, pol=True, chan=True)
...
for basefile in basefiles:
    ...
    srcant = {}
    for k in srcs:
        uvi = a.miriad.UV(basefile)
        uvo = a.miriad.UV(basefile+'E_'+k,status='new')
        print basefile, "->", basefile+'E_'+k
        uvo.init_from_uv(uvi)
        chans = a.scripting.parse_chans(opts.chan, uvi['nchan'])
        nchan = range(uvi['nchan'])
        srcant[k] = {}
        for i in srcs[k]:
            _ant, _wgt = 0, 0
        ...
        for i in srcant.get(k,{}):
          for j in srcant.get(k,{}):
            ...
            uvw = float(abs(j - i))
            for x,y in enumerate(d[a.miriad.ij2bl(i,j)]):
                data = []
                flags = []
                cnt = 0
                for e,f in enumerate(nchan):
                    if f in chans:
                        data.append(y[cnt])
                        flags.append(0)
                        cnt = cnt + 1
                ...
                flags = n.array(flags)
                p = (n.array([uvw,uvw,uvw]),times[x],(i,j))
                uvo.write(p,data,flags=flags)
        sys.exit()
        flag = False
        for bl in d:
