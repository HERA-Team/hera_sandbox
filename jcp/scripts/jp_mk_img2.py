...
import aipy as a, numpy as n, sys, optparse, ephem, os, math as m
...
if opts.src == 'zen':
    ...
    #ra_dec=n.column_stack((ras,decs))
    #nra_dec=n.empty(n.shape(ra_dec))
    #rot_ax=a.coord.radec2eq((0,0))
    #rot_mat=a.coord.rot_m(m.pi/4,rot_ax)
    #i=0
    #for k in ra_dec:
    #    vec=a.coord.radec2eq(k)
    #    rvec=n.dot(vec,rot_mat)
    #    nra_dec[i]=a.coord.eq2radec(rvec)
    #    i=i+1
    #ras,decs=n.hsplit(nra_dec,2) 
    ras=n.array(ras)
    ras=ras+(m.pi/10)
    mask=n.greater(ras, 2*m.pi)
    n.putmask(ras,mask,ras-(2*m.pi))
