...
for uvfile in args:
    ...
    for pol in data:
      for bl in data[pol]:
        ...
        cd = ad.flatten().compress(n.logical_not(m.flatten()))
        med = n.median(cd)
        sig = n.sqrt(n.median((cd-med)**2))
    ...
    uvo.pipe(uvi, mfunc=rfi_mfunc, raw=True, append2hist='XRFI_SIMPLE: nsig=%f combine=%s thresh=%d chans=%s\n' %  (opts.nsig, opts.combine, opts.thresh, opts.chan))
