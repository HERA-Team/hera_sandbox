...
import optparse, sys, os,traceback
...
o.add_option('-n','--nsig',default=2,type='float',
    help='Number of standard deviations above mean to flag if neither --dt nor -df are specified. [2]')
...
#        if bl == 257: print "!"
    ...
    def rfi_mfunc(uv, preamble, data, flags):
        ...
        try:m = mask[uv['pol']][bl][t]
            ...
            return preamble, n.zeros_like(data),n.ones_like(data)
            print mask[uv['pol']].keys()
            print uv['pol'],bl,t,':',traceback.print_exc();sys.exit()
    ...
    uvo.pipe(uvi, mfunc=rfi_mfunc, raw=True, append2hist='XRFI_SIMPLE: chan=%s df=%s dt=%s combine=%s thresh=%d\n' % (opts.chan, str(opts.df), str(opts.dt), opts.combine, opts.thresh))
