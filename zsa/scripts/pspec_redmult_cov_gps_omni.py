...
aa = a.cal.get_aa('psa6240_v003', .1/203, .1, 203) #bogus params in get_aa
...
sdic = capo.dfm.grid2ij(ANTPOS)[0]

def ubl2bl(ubl):
    '''Given omni cal unique bl separation, 
       Find baeline that represents that.'''
    
    sn = int(ubl.split('p')[-1])
    sepy,sepx = ubl2sep[sn][:2]
    bl = sdic[','.join([str(int(-1*sepy)),str(int(sepx))])]     
    return map(int, bl.split(',')[0].split('_'))
...
import frf_conv
b2,b1 = ubl2bl(opts.sep)
beam_w_fr = frf_conv.get_beam_w_fr(aa, (b1,b2)) #I have been using (49,41)
t, firs, frkers = frf_conv.get_fringe_rate_kernels(beam_w_fr, 42.8, 400)
firs = firs.take(chans, axis=0)
#normalize the fir filters since they are on different scales.o
import IPython
IPython.embed()
#print t
...
for filename in args:
    ...
    for ch in xrange(d.shape[1]):
        d[:,ch] = n.convolve(w[:,ch]*d[:,ch], n.conj(firs[ch,:]), mode='same')
        w[:,ch] = n.convolve(w[:,ch], n.abs(n.conj(firs[ch,:])), mode='same')
        d[:,ch] /= n.where(w[:,ch] > 0, w[:,ch], 1)
    w = n.where(w > .1, 1, 0)
    d *= w
    capo.arp.waterfall(d)
    p.show()
    d = d[300:800] # XXX
    w = w[300:800] # XXX
