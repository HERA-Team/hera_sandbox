...
for jj, f in enumerate(freqs):
    ...
    fng = numpy.exp(-2j*numpy.pi*(blx*tx+bly*ty*numpy.sqrt(1-tz**2))*f) #fringe pattern
...
#    plt.subplots_adjust(left=0.1, right=0.9, bottom=0.1, top=0.9, wspace=0.4, hspace=0.3)
    ...
    for ii, t in enumerate(times):
        ...
        if plt1 == None:
            pylab.subplot(2,1,1)
            plt1 = pylab.plot(times,toplot1,'b-')
            pylab.xlabel('Time')
            pylab.ylabel('Sum(Fluxes)')
            pylab.ylim(0,2)
            pylab.subplot(2,1,2)
            plt2 = pylab.plot(times,toplot2,'r-')
            #plt3 = plt.plot(times,toplot3,'b-')
            pylab.xlabel('Time')
            pylab.ylabel('Real(Vis)')
            pylab.ylim(-3e-6,3e-6)
            pylab.draw()
            pylab.pause(.5)
        else:
