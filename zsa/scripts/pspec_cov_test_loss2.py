...
AVG_COV = True
LST_STATS = False
#of the noise. Signal always stays the same and we just vary the noise.
GAIN=10.
ID=False
BLOCK_DIAG=True
...
if False: scalar = capo.pspec.X2Y(z) * bm * B
else: scalar = 1
if not DELAY:
    # XXX this is a hack
...
for k in signal:
    ...
    data[k]['I'] = signal[k]['I'] + GAIN*noise[k]['I']
    if DELAY: 
...
for boot in xrange(opts.nboot):
    ...
    if MASK: # mask covariance matrix
        if BLOCK_DIAG:
            mask = n.zeros((nbls,nchan,nbls,nchan))
            for i in n.arange(nbls):
                mask[i,:,i,:] = 1
        else:
...
#        p.plot(kpl, leftover, 'r.-')
        ...
        p.figure(2)
        capo.arp.waterfall(C); p.colorbar(shrink=.5)
        p.figure(3)
        capo.arp.waterfall(_C); p.colorbar(shrink=.5)
        p.show()
