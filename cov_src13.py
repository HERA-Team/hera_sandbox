...
for arg in args:
    ...
    print shape,
    padlen = math.ceil(shape[1] * .2)
    shape = (shape[0], shape[1] + 2*padlen)
    print shape
    fft_ddr_dat = n.zeros((shape[0],shape[1]), dtype=n.complex)
    ...
    for iter in xrange(opts.maxiter):
        ...
        if mode == 0: # BEAMFORM
            for k in simdat:
                ...
                d = fft_ddr_dat[:,padlen:-padlen] / (shape[0] * shape[1])
            ...
            for k in simdat:
                ...
                for i in d:
                    ...
                    _d = n.concatenate([n.fliplr(d[i][:,:padlen]),d[i],n.fliplr(d[i][:,-padlen:])], axis=1)
                    fft_ddr_dat[:] = _d
                    fft_ddr_rev() # DDR Transform
                    # Apply DDR Filter
                    x1,x2 = drw, -drw
                    ...
                    fft_ddr_dat[:,y1+1:y2] = 0
                    fft_ddr_fwd() # undo DDR Transform
                    d[i] = fft_ddr_dat[:,padlen:-padlen] / (shape[0] * shape[1])
