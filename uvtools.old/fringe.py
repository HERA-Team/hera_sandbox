...
DEFAULT_FRBINS = np.arange(-.01+5e-5/2,.01,5e-5) # Hz
#DEFAULT_FRBINS = np.arange(-.5/42.94999+5e-5/2,.5/42.94999,5e-5) # Hz old fr_bins, new inttime
DEFAULT_WGT = lambda bm: bm**2
DEFAULT_IWGT = lambda h: np.sqrt(h)
...
def gen_frbins(inttime, fringe_res=5e-5):
    ...
    """
    fmax = 0.5 / inttime
    return np.arange(-fmax, fmax+fringe_res/2, fringe_res)

def mk_fng(bl, eq):
    '''Return fringe rates given eq coords and a baseline vector (measured in wavelengths) in eq coords'''
    return -2*np.pi/aipy.const.sidereal_day * np.dot(np.cross(np.array([0,0,1.]),bl), eq)
...
def mk_fng_alietal(bl, eq):
    ...
    return 2*np.pi/aipy.const.sidereal_day * (bl[0]*ex + bl[1]*ey * np.sqrt(1-ez**2))
...
def fr_profile(bm, fng, bins=DEFAULT_FRBINS, wgt=DEFAULT_WGT, iwgt=DEFAULT_IWGT):
    ...
    h, _ = np.histogram(fng, bins=bins, weights=wgt(bm))
    h = iwgt(h)
    h /= h.max()
...
def gauss(cenwid, bins): return np.exp(-(bins-cenwid[0])**2/(2*cenwid[1]**2))
def tanh(x, p, w, C = 1.0, a=1.0): return (C/2.) * (1 + a*np.tanh( (x-p)/(2*w)))
def mdl_wrap(prms, frp, bins, maxfr, mdl): return np.sum((frp - np.where(bins > maxfr,0,mdl(prms,bins)))**2)
...
def hmap_to_fr_profile(bm_hmap, bl, lat, bins=DEFAULT_FRBINS, wgt=DEFAULT_WGT, iwgt=DEFAULT_IWGT,alietal=False):
    ...
    eq = bm_hmap.px2crd(np.arange(bm_hmap.npix()), ncrd=3) # equatorial coordinates
    eq2zen = aipy.coord.eq2top_m(0., lat)
    top = np.dot(eq2zen, eq)
    _bm = bm_hmap[(top[0], top[1], top[2])]
    _bm = np.where(top[2] > 0, _bm, 0)
    bm = _bm
    if alietal:
...
def aa_to_fr_profile(aa, i, j, ch, pol='I', bins=DEFAULT_FRBINS, wgt=DEFAULT_WGT, iwgt=DEFAULT_IWGT, nside=64, bl_scale=1, alietal=False, **kwargs):
    ...
    eq = h.px2crd(np.arange(h.npix()), ncrd=3)
    top = np.dot(aa._eq2zen, eq)
    if alietal:
    ...
    _bmx = aa[0].bm_response((top), pol='x')[ch]; _bmx = np.where(top[2] > 0, _bmx, 0)
    _bmy = aa[0].bm_response((top), pol='y')[ch]; _bmy = np.where(top[2] > 0, _bmy, 0)
...
def normalize(fx):
    return fx / np.sqrt(np.sum(np.abs(fx)**2,axis=-1))
...
def frp_to_firs(frp0, bins, fqs, fq0=.150, limit_maxfr=True, limit_xtalk=True, fr_xtalk=.00035, maxfr=None,
    ...
    frps = np.array([mdl(prms0*fq/fq0,bins) * limit_maxfr(fq) * limit_xtalk for i,fq in enumerate(fqs)])
    tbins = fftshift(fftfreq(bins.size, bins[1]-bins[0]))
    firs = frp_to_fir(frps)
    firs *= aipy.dsp.gen_window(bins.size, window)
    if alietal:
        firs /= np.sum(np.abs(firs),axis=1).reshape(-1,1) # normalize so that np.sum(abs(fir)) = 1
    else:
        firs /= np.sqrt(np.sum(np.abs(firs)**2,axis=1).reshape(-1,1)) # normalize so that np.sum(abs(fir)**2) = 1
    return tbins, firs
...
def apply_frf(aa, data, wgts, i, j, pol='I', firs=None, alietal=False,
    ...
    datf,wgtf = np.zeros_like(data), np.zeros_like(data)
    fir = firs[(i,j,pol)]
    for ch in xrange(nchan):
        #datf[:,ch] = np.convolve(data[:,ch], fir[ch,:], mode='same')
        #wgtf[:,ch] = np.convolve(wgts[:,ch], np.abs(fir[ch,:]), mode='same')
        datf[:,ch] = np.convolve(data[:,ch]*wgts[:,ch], np.conj(fir[ch,:]), mode='same')
        wgtf[:,ch] = np.convolve(wgts[:,ch], np.abs(np.conj(fir[ch,:])), mode='same')
    datf = np.where(wgtf > 0, datf/wgtf, 0)
