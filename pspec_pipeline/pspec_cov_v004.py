...
import sys
import random
from capo import zsa as zsa, oqe as oqe, frf_conv as fringe
...
o.add_option('--window', dest='window', default='blackman-harris',
             help=('Windowing function to use in delay transform.  Default is'
                   ' blackman-harris.  Options are: ' +
                   ', '.join(a.dsp.WINDOW_FUNC.keys())))
...
def get_Q(mode, n_k):
    """Encode the fourier transform from freq to delay."""
    if not DELAY:
...
lsts, data, flgs = {}, {}, {}
for k in days:
    lsts[k], data[k], flgs[k] = capo.miriad.read_files(dsets[k], antstr=antstr,
                                                       polstr=POL,
                                                       verbose=True)
...
ds = oqe.DataSet()
lsts, data_dict, flg_dict = ds.lst_align(lsts, dsets=data_dict, wgts=flg_dict)
...
if opts.noise_only:
    ...
    if opts.same:
        ...
        # same noise on all bls
    for key in data_dict:
        if opts.same:
            thing = NOISE.T
        if opts.diff:
            thing = frf((len(chans), timelen), loc=0, scale=1).T
...
for boot in xrange(opts.nboot):
    ...
    if len(opts.output) > 0:
        outpath = opts.output + '/pspec_boot%04d.npz' % boot
    else:
        outpath = 'pspec_boot%04d.npz' % boot
