...
for k in days:
    ...
    for bl in data[k]:
        ...
        if opts.noise_only:
            xi[k][bl] = NOISE #frf((len(chans),len(lsts)),loc=0,scale=1e7) #diff noise for each bl
        else:
...
#XXX Calculate pC just based on the simulation noise (no eor injection)
print 'Getting pCv'
x = {}
for k in days:
    x[k] = {}
...
#XXX Loop to calculate pC of (noise+eor) and pI of eor
