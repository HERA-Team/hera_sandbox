#! /usr/bin/env python
"""Simulates Singal Loss by injecting known signals into data."""
import aipy as a
import numpy as n
import pylab as p
import capo
...
o.add_option('--plot', action='store_true', help='Generate plots to X-window')
...
def cov(m):
    """Compute Covariance for Complex numbers.
...
def get_Q(mode, n_k):
    ...
    Generates matrices used for Fourier Transforms.
    """
    if not DELAY:
...
dsets = {
    ...
    # 'even': glob.glob('/data4/paper/2013EoR/Analysis/ProcessedData/'
    #                    'epoch2/omni_v2_xtalk/lstbin_manybls/even/'
    #                    'sep0,2/*I.uvGAL'),
    # 'odd': glob.glob('/data4/paper/2013EoR/Analysis/ProcessedData/'
    #                   'epoch2/omni_v2_xtalk/lstbin_manybls/odd/'
    #                   'sep0,2/*I.uvGAL')
    # 'even': glob.glob('/Users/sherlock/projects/paper/analysis/psa64/'
    #                   'lstbin_even_xtalk_removed/' + SEP +
    #                   '/newfrf/*242.[3456]*uvGL'),
    # 'odd' : glob.glob('/Users/sherlock/projects/paper/analysis/psa64/'
    #                    'lstbin_odd_xtalk_removed/' + SEP +
    #                    '/newfrf/*243.[3456]*uvGL'),
        }
# for i in xrange(10):
#    dsets[i] = glob.glob('lstbinX%d/%s/lst.24562[45]*.[3456]*.uvAL'%(i,SEP))
...
#  XXX hack for *E files that have inttime set incorrectly
...
# Acquire data
# antstr =('41_49,3_10,9_58,22_61,20_63,2_43,21_53,'
#          '31_45,41_47,3_25,1_58,35_61,42_63,2_33')
antstr = 'cross'
lsts, data, flgs = {}, {}, {}
days = dsets.keys()
for k in days:
...
if LST_STATS:
    ...
    for filename in dsets.values()[0]:
        ...
        for (uvw, t, (i, j)), d, f in uv.all(raw=True):
            if len(times) == 0 or times[-1] != uv['lst']:
                times.append(uv['lst'])
            bl = '%d,%d,%d' % (i, j, uv['pol'])
...
for boot in xrange(opts.nboot):
    ...
    # get Q matrix (does FT from freq to delay)
    I, _I, _Ix = {}, {}, {}
    C, _C, _Cx = {}, {}, {}
    for k in days:
        I[k], _I[k], _Ix[k] = {}, {}, {}
    ...
    for k in days:
        ...
        for i, gp in enumerate(gps):  # sum things up over the groups
            ...
            for ch in xrange(nchan):
                # XXX this loop makes computation go as nchan^3
        ...
        if PLOT:
            ...
            print _Czk.shape
            _Czk = n.reshape(_Czk,
                             (_Czk.shape[0] * _Czk.shape[1], _Czk.shape[2]))
            p.subplot(211)
            capo.arp.waterfall(_Czk, mode='real')
            p.subplot(223)
            capo.arp.waterfall(_Csumk)
            p.subplot(224)
            capo.arp.waterfall(cov(_Czk))
            p.show()
    print "   Getting F and q"
    FI = n.zeros((nchan, nchan), dtype=n.complex)
    FC = n.zeros((nchan, nchan), dtype=n.complex)
    qI = n.zeros((nchan, _Iz.values()[0].values()[0].shape[1]),
                 dtype=n.complex)
    qC = n.zeros((nchan, _Cz.values()[0].values()[0].shape[1]),
                 dtype=n.complex)
    Q_Iz = {}
    Q_Cz = {}
    for cnt1, k1 in enumerate(days):
        for k2 in days[cnt1:]:  # loop over even with eve, even with odd, etc.
            if k2 not in Q_Iz.keys():
                Q_Iz[k2] = {}
            if k2 not in Q_Cz.keys():
                Q_Cz[k2] = {}
            for bl1 in _Cz[k1]:
                for bl2 in _Cz[k2]:
                    if k1 == k2 or bl1 == bl2:
                    ...
                    if PLOT and False:
                        p.subplot(231)
                        capo.arp.waterfall(C[m], drng=3)
                        p.subplot(232)
                        capo.arp.waterfall(_C[m], drng=3)
                        p.subplot(233)
                        capo.arp.waterfall(n.dot(C[m], _C[m]), drng=3)
                        p.subplot(234)
                        p.semilogy(S)
                        p.subplot(236)
                    ...
                    if False:  # use ffts to do q estimation fast
                        ...
                        if bl2 not in Q_Iz[k2].keys():
                            Q_Iz[k2][bl2] = [n.dot(Q[i], _Iz[k2][bl2])
                                             for i in xrange(nchan)]
                        if bl2 not in Q_Cz[k2].keys():
                            Q_Cz[k2][bl2] = [n.dot(Q[i], _Cz[k2][bl2])
                                             for i in xrange(nchan)]
                        _qI = n.array([_Iz[k1][bl1].conj() * Q_Iz[k2][bl2][i]
                                      for i in xrange(nchan)])
                        qI += n.sum(_qI, axis=1)
                        _qC = n.array([_Cz[k1][bl1].conj() * Q_Cz[k2][bl2][i]
                                      for i in xrange(nchan)])
                        # C^-1 Q C^-1
                        qC += n.sum(_qC, axis=1)
                    if DELAY:
                        # by taking FFT of CsumQ above
                        # each channel is already i,j separated
                        FI += n.conj(_IsumQ[k1][bl1]) * _IsumQ[k2][bl2]
                        FC += n.conj(_CsumQ[k1][bl1]) * _CsumQ[k2][bl2]
                    else:
                        for i in xrange(nchan):
                            for j in xrange(nchan):
                                FI[i, j] += n.einsum('ij,ji',
                                                     _IsumQ[k1][bl1][i],
                                                     _IsumQ[k2][bl2][j])
                                FC[i, j] += n.einsum('ij,ji',
                                                     _CsumQ[k1][bl1][i],
                                                     _CsumQ[k2][bl2][j])
                                #  C^-1 Q C^-1 Q
    ...
    if INJECT_SIG > 0.:  # Create a fake EoR signal to inject
        ...
        for k in days:
            x[k] = {}
            for bl in xi[k]:
                x[k][bl] = xi[k][bl].copy() + eor.copy()
                # add injected signal to data
        if False and PLOT:
            p.subplot(211)
            capo.arp.waterfall(eor1, mode='real')
            p.colorbar()
            p.subplot(212)
            capo.arp.waterfall(eor2, mode='real')
            p.colorbar()
            p.show()
    Q = [get_Q(i, nchan) for i in xrange(nchan)]
    # get Q matrix (does FT from freq to delay)
    ...
    for k in days:
        ...
        for bl in bls_master:
            ...
            _C[k][bl] = n.einsum('ij,j,jk', V.T, 1./S, U.T)
            _I[k][bl] = n.identity(_C[k][bl].shape[0])
            # _C[k][bl] = n.identity(_C[k][bl].shape[0]) # overwriting C with I
    ...
    for k in days:
        ...
        for i, gp in enumerate(gps):  # sum things up over the groups
            ...
            _IsumQ[k][i] = {}
            _CsumQ[k][i] = {}
            # this next loop makes computation go as nchan^3
            for ch in xrange(nchan):
                _IsumQ[k][i][ch] = n.dot(_Isum[k][i], Q[ch])
                _CsumQ[k][i][ch] = n.dot(_Csum[k][i], Q[ch])
        if PLOT:
            ...
            for bl1 in _Cz[k1]:
                for bl2 in _Cz[k2]:
                    ...
                    if False:  # use ffts to do q estimation fast
                        ...
                        qI += n.sum(_qI, axis=1)
                        _qC = n.array([_Cz[k1][bl1].conj() * Q_Cz[k2][bl2][i]
                                       for i in xrange(nchan)])  # C^-1 Q C^-1
                        qC += n.sum(_qC, axis=1)
                    if DELAY:
                        # by taking FFT of CsumQ above
                        # each channel is already i,j separated
