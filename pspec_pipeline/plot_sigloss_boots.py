...
def gauss(x,u,s): return n.exp(-(x-u)**2/(2.*s**2))
...
for inject in files:
    ...
    kpls.append(_kpls)
    pCs.append(pC_avg) #should bootstrap all these pC_avg's (20 of them) to get error bars... look at their distribution
    pIs.append(pI_avg)
    pIvs.append(pIv_avg)
...
pIs,pCs,pCvs = n.array(pIs), n.array(pCs), n.array(pCvs) #avg over inject #s
pIvs = n.array(pIvs)
...
pCvs = n.reshape(pCvs, (pCvs.shape[0],pCvs.shape[2],pCvs.shape[1],pCvs.shape[3]))
pIvs = n.reshape(pIvs, (pIvs.shape[0],pIvs.shape[2],pIvs.shape[1],pIvs.shape[3]))
#P(k) values have k, inj, boots, times
pCvs_pk = n.reshape(pCvs_pk, (pCvs_pk.shape[2],pCvs_pk.shape[0],pCvs_pk.shape[1],pCvs_pk.shape[3]))
pIvs_pk = n.reshape(pIvs_pk, (pIvs_pk.shape[2],pIvs_pk.shape[0],pIvs_pk.shape[1],pIvs_pk.shape[3]))
...
n_fqs, n_ks, noise = py21cm.load_noise_files(
        glob.glob('/home/mkolopanis/psa64/21cmsense_noise/new_noise/*drift_mod*.npz'))
...
d2_n =  2* noise_interp( freq*1e3, n_k )
pk_n = 2*n.pi**2/(n_k**3) * d2_n
p_n = n.median( pk_n )
...
for nboot in xrange(NBOOT):
    if (nboot+1) % 10 == 0: print '   ',nboot+1,'/',NBOOT
    dsum_pIvs = []
    dsum_pCvs = []
    dsum_pCs = []
    dsum_pIs = []
    for cnt in xrange(pIvs.shape[-1]):
    ...
    pIvs_boot.append(pIvs_sum.T)
    pCvs_boot.append(pCvs_sum.T)
    pIs_boot.append(pIs_sum.T)
    pCs_boot.append(pCs_sum.T)
...
'''
# d2_boot = n.reshape(d2_boot,(n.shape(d2_boot)[1], n.shape(d2_boot)[0]*n.shape(d2_boot)[-1]))
...
# p.fill_between([pklo,pkhi],[pkdn,pkdn],[pkup,pkup], facecolor='gray', edgecolor='gray',alpha=.4)
"""p
...
p.errorbar((n.abs(pIs)+ n.abs(p_n))/n.abs(pCs),n.abs(pCs),
        (n.abs(pIs)+ n.abs(p_n))/(pCs)**2*pC_err + n.abs(pI_err)/n.abs(pCs),fmt='k.')
p.errorbar((n.abs(pIs)+ n.abs(p_n))/n.abs(pCs) ,n.abs(pCvs), pCv_err,fmt='.',color='blue',alpha=.2)
...
p.fill_between([1e-3,1e8], [pkdn,pkdn], [pkup,pkup], facecolor='gray', edgecolor='gray',alpha=.4)
sig_factors = []
...
p.errorbar(pIs,n.abs(pCs)/(n.abs(pIs) + n.abs(p_n)),
    pC_err/(n.abs(pIs) + n.abs(p_n)) + pCs/(n.abs(pIs) + n.abs(p_n))**2*pI_err,fmt='k.')
ax0.set_xscale('log')
p.xlim(pklo, pkhi)
#p.ylim(3e-2,3)
p.ylim(1e-5,1e0)
...
ascii.write(table, 'sigloss_limits.dat', names=[ 'P_inj' ,'Lower','Median','Upper'])

pC_upper = pCs + pC_err[1]
pCv_upper = pCvs + pCv_err[1]

# p_eor = n.max([pIs[loc] for loc,val in enumerate(pC_tot) if val <= pCv_tot[loc]])
p_inj_5 = interp1d(pC_upper, pIs,kind='linear',bounds_error=False,fill_value=0,
                    assume_sorted=False)
