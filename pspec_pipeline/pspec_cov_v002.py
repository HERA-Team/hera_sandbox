...
o.set_description('http://imgs.xkcd.com/comics/code_quality.png')
...
bm = n.polyval(capo.pspec.DEFAULT_BEAM_POLY, afreqs)
...
if True:
    ...
    lstmax = max([lsts[k][0] for k in days]) #of all the seperate days, this latest "first lst" 
    for k in days:
        print k
        for i in xrange(len(lsts[k])): #chop off lsts below the max of the mins
    ...
    #chop off any lsts hanging off at the end too
    j = min([len(lsts[k]) for k in days])
    for k in days:
        lsts[k] = lsts[k][:j]
        for bl in data[k]:
            data[k][bl],flgs[k][bl] = n.array(data[k][bl][:j]),n.array(flgs[k][bl][:j])
    print "maximum deviation in lst is ",n.max(n.diff([lsts[k] for k in days],axis=0)),"r"
else:
    for k in days:
        for bl in data[k]:
...
bls_master = n.sort(x.values()[0].keys())
nbls = len(bls_master)
...
rows = n.ceil(n.sqrt(len(d)))
cols = n.ceil(len(d)/rows)


#compute the baseline difference covariance
Nt,sigma,dC,_dC = {},{},{},{}
Nt_fo_real = {}
means = []
for i,bl in enumerate(bls_master):
    dC[bl] = cov(d[bl])
    U,S,V = n.linalg.svd(dC[bl])
    sigma[bl] = n.mean(S)
    res = dC[bl]-n.identity(nchan)*sigma[bl]
    Nt[bl] = n.max([int((sigma[bl]/n.median(n.abs(res)))**2/n.sqrt(2)),nchan])
    dlst = n.ceil(d[bl].shape[1]/Nt[bl])
    dlst =n.ceil(d[bl].shape[1]/13.)
    Nt_fo_real[bl] = int((sigma[bl]/n.median(n.abs(res)))**2/n.sqrt(2))
    #_dC[bl] = n.einsum('ij,j,jk',V.T,1./S,U.T)
    if PLOT and False:
        p.figure(10)
        p.subplot(rows,cols,i+1)
        #capo.arp.waterfall(d[bl],mode='real')
        p.hist(d[bl].ravel().real,bins=100,histtype='step')
        print "  ",bl,"real mean [mK]",n.mean(d[bl][:,::dlst].real),
        means.append(n.mean(d[bl][:,::dlst].real))
        print "std [mK]",n.std(d[bl][:,::dlst].real),
        print "error [mK] ",n.std(d[bl][:,::dlst].real)/n.sqrt(Nt[bl]*nchan)
    ...
    if PLOT and False:
        ...
        capo.arp.waterfall(d[bl],mode='real')
        p.twinx()
        p.plot(n.mean(d[bl]*n.conj(d[bl]),axis=0))
        p.subplot(234)
        capo.arp.waterfall(dC[bl])
        p.subplot(235)
        p.plot(n.einsum('ij,jk',n.diag(S),V).T.real)
        p.plot(n.sort(S),'.k')
        #make a model of the noise
        p.subplot(236)
        Sigma = n.mean(S)
        res = dC[bl]-n.identity(nchan)*Sigma
        fitted_Nt = int((Sigma/n.median(n.abs(res)))**2/n.sqrt(2))
        print "estimated number of free parameters:",fitted_Nt
        fitted_Nt = int(n.max([fitted_Nt,nchan]))
        print "rounded number",fitted_Nt
        capo.arp.waterfall(dC[bl]-n.identity(nchan)*Sigma,mode='real')
        print "Nres>0",n.sum(res.real>0)
        print "Nres<0",n.sum(res.real<0)
        print "Meanres",n.mean(res)
        NC_model = stats.wishart.rvs(df=fitted_Nt,scale=n.identity(nchan)*Sigma)/(fitted_Nt)
        U,S_model,V = n.linalg.svd(NC_model)
        p.subplot(235)
        p.plot(n.sort(S_model),'-k')
        p.suptitle('%d_%d'%a.miriad.bl2ij(bl))
        p.show()
print "min,mean,max Nt",n.min(Nt_fo_real.values()),n.mean(Nt_fo_real.values()),n.max(Nt_fo_real.values())
if PLOT:p.show()
...
for k in days:
    ...
    for bl in x[k]:
        C[k][bl] = cov(x[k][bl])
        if False: #subtract infinite sample noise covariance 
        ...
        if PLOT and True:
            ...
            p.subplot(345); capo.arp.waterfall(C[k][bl])
            p.subplot(346); p.plot(n.einsum('ij,jk',n.diag(S),V).T.real)
            p.plot(n.sort(S),'.k')
            p.plot(n.sort(SN),'-k')
            print 1/S
            p.subplot(347);p.semilogy(n.sort(S/SN),'.')
            p.subplot(348); capo.arp.waterfall(_C[k][bl])
            p.subplot(313); capo.arp.waterfall(_Cx[k][bl], mode='real')
            p.suptitle('%d_%d'%a.miriad.bl2ij(bl))
#            p.figure(2); p.plot(n.diag(S))
            print "="*10
            print "number of zeros 1% cut",n.sum(S<(n.max(S)*0.01))
            if Nt_fo_real[bl]>nchan:
                print "matrix is full rank! with Nt=",Nt_fo_real[bl]
            else:    
                print "estimated number of zeros (Nt-1)",nchan - (Nt_fo_real[bl]-1)
                print "dynamic range cut for noise subspace",n.max(S)/n.sort(S)[(Nt_fo_real[bl]-1)]
                print "SNR level of noise subspace",n.sort(S)[(Nt_fo_real[bl]-1)]/n.sort(SN)[(Nt_fo_real[bl]-1)]
            p.show()
...
for boot in xrange(opts.nboot):
    ...
    for k in days:
        ...
        for i,gp in enumerate(gps):
            ...
            _Cz[k][i] = sum([_Cx[k][bl] for bl in gp])#Eq 22
        ...
        if PLOT:
            ...
            #_Csum[k] = _Csumk
            _Czk = n.array([_Cz[k][i] for i in _Cz[k]])#_Cz arrayed over groups
            _Czk = n.reshape(_Czk, (_Czk.shape[0]*_Czk.shape[1], _Czk.shape[2]))
            p.subplot(211); capo.arp.waterfall(_Czk, mode='real')
            p.title('eq 22, like bottom right of Fig 13')
            p.subplot(223); capo.arp.waterfall(_Csumk)
            p.title('1/C averaged over baselines within groups')
            p.subplot(224); capo.arp.waterfall(cov(_Czk))
            p.title('cov of top. kinda like a residual')
            p.figure()
            ...
            for bl1 in _Cz[k1]:
                for bl2 in _Cz[k2]:
                    ...
                    if False: # use ffts to do q estimation fast
                        ...
                        qI += n.sum(_qI, axis=1)
                        _qC = n.array([_Cz[k1][bl1].conj() * Q_Cz[k2][bl2][i] for i in xrange(nchan)]) #eq 25
                        qC += n.sum(_qC, axis=1)
                    if DELAY: # by taking FFT of CsumQ above, each channel is already i,j separated
                        ...
                        for i in xrange(nchan):
                            for j in xrange(nchan):
                                ...
                                FC[i,j] += n.einsum('ij,ji', _CsumQ[k1][bl1][i], _CsumQ[k2][bl2][j]) #eq 17
