...
for cnt,pad in enumerate(frpads):
    ...
    for sep in seps:
        ...
        #startparms=(.001*pad,.0001)
        #startparms= tuple([pad*par for par in startparms])
        #timebins, firs[sep] = fringe.frp_to_firs(frp, bins, aa.get_afreqs(), fq0=aa.get_afreqs()[mychan], limit_xtalk=True,startprms=startparms,frpad=opts.frpad)
