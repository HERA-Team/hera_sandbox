...
print avg_nk_2d.shape, wgts.shape
...
for boot in xrange(NBOOT):
    ...
    for t in xrange(2*ntimes):
        ...
        if h == 0:
            ...
            nsum_fold += [nk_2d[b,k0+1:,t] * wgts[k0+1:,t]]
            dwgt_fold += [wgts[k0+1:,t]]
        else:
            dsum_fold += [pk_2d[b,k0-1::-1,t] * wgts[k0-1::-1,t]]
            nsum_fold += [nk_2d[b,k0-1::-1,t] * wgts[k0-1::-1,t]]
    ...
    nk_fold_boot.append(nsum_fold / dwgt_fold)
...
if True:
    ...
    for k_,pk_,err_ in zip(kpl, nk/1e6, 2*nerr/1e6):
        print '%6.3f, %9.5f (%9.5f +/- %9.5f)' % (k_, pk_+err_,pk_,err_)
    print '-'*70
    for k_,pk_,err_ in zip(kpl[k0:], nk_fold/1e6, 2*nerr_fold/1e6):
        print '%6.3f, %9.5f (%9.5f +/- %9.5f)' % (k_, pk_+err_,pk_,err_)
else:
...
n.savez('pspec.npz', kpl=kpl, pk=pk, err=err, pk_fold=pk_fold, err_fold=err_fold, cmd=cmd,afreqs=afreqs,chans=chans, nk=nk, nk_fold=nk_fold, nerr=nerr, nerr_fold=nerr_fold)
