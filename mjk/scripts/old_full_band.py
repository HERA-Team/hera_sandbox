...
tmp_noise[k1]={}
tmp_noise[k2]={}
for ble,blo in zip(data[k2].keys(),data[k1].keys()):
    tmp_noise[k1][blo] = n.copy(data[k1][blo] - data[k2][ble])
    tmp_noise[k2][ble] = n.copy(data[k1][blo] - data[k2][ble])
...
for k in days:
    ...
    for bl in data[k]:
        ...
        dn = (tmp_noise[k][bl][:,band_chans] * jy2T)[:,index]
        #d1 = n.zeros_like(d)
        #d1[:,index] +=  d[:,index]
        d1= n.copy(d)
        d[:,index] = 0. + 0j
...
for boot in xrange(opts.nboot):
    ...
    _Iz,_Isum,_IsumQ,_Inz = {},{},{},{}
    _Cz,_Csum,_CsumQ = {},{},{}
    Csum = {}
    for k in days:
        _Iz[k],_Isum[k],_IsumQ[k], _Inz[k] = {},{},{},{}
    ...
    FN = n.zeros((nchan,nchan), dtype=n.complex)
    ...
    MN  = n.identity(nchan, dtype=n.complex128)
    ...
    WN = n.dot(MN, FN)
    norm  = WN.sum(axis=-1); norm.shape += (1,)
    #norm  = WI.max(axis=-1); norm.shape += (1,) # XXX
    MN /= norm; WN = n.dot(MN, FN)
    WC = n.dot(MC, FC)
