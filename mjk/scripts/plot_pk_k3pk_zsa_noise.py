...
for filename in args:
    ...
    if False: # Hacky way to get a noise bias out, if necessary
        ...
        nk -= n.median(n.concatenate([nk[:8], pk[-8:]]))
    ...
    for _kpl, _pk, _err in zip(kpl, nk, nerr):
        NS_VS_KPL[filename][_kpl] = (_pk, _err)
        nsum[_kpl] = nsum.get(_kpl, 0) + _pk / _err**2
    ...
    if not nkfold is None:
        for _kpl, _pk, _err in zip(kpl[k0:], nkfold, nerrfold):
            NS_VS_KPL_FOLD[filename][_kpl] = (_pk, _err)
            nsum_fold[_kpl] = nsum_fold.get(_kpl, 0) + _pk / _err**2
#freq = f['freq']
...
if True:
    ...
    for _kpl in nsum:
        NS_VS_KPL['total'][_kpl] = (nsum[_kpl] / dwgt[_kpl], 1./n.sqrt(dwgt[_kpl]))
    for _kpl in nsum_fold:
        NS_VS_KPL_FOLD['total'][_kpl] = (nsum_fold[_kpl] / dwgt_fold[_kpl], 1./n.sqrt(dwgt_fold[_kpl]))
...
#        f = 1.15 # this is the conservative number, for the biggest modes outside the of the wedge. Get ~1.03 correction at k=.2
    ...
    if dn_fold.size == 0: dn_fold = None
