...
for filename in args:
    ...
    afreqs = f['afreqs']
    chans = f['chans']
    c_num_min = f['condition_min']
    c_num_max = f['condition_max']
...
pI_2d = n.array([nocov_vs_t[path] for path in paths]) #(bltype,bootstraps,kpls,times), T averaged over all bls
...
avg_pI_2d = n.average(pI_2d, axis=1) #(bltype,kpls,times), best estimate of pk in each integration
...
if CLIP: #trim time axis
    ...
    wgts = wgts[...,LO:HI]
    pI_2d = nocov_2d[...,LO:HI]
    avg_pI_2d = avg_pI_2d[...,LO:HI]
    wgts_pI = wgts_pI[...,LO:HI]
else:
    pass
...
pI_2d = pI_2d.transpose([1,2,3,0]).copy() #(bootstraps,kpls,times,bltype)
pI_2d.shape = pI_2d.shape[:-2] + (pI_2d.shape[-2] * pI_2d.shape[-1],) #(bootstraps,kpls,timebls)
wgts = wgts.transpose([1,2,0]).copy() #(kpls,times,bltypes)
wgts.shape = wgts.shape[:-2] + (wgts.shape[-2] * wgts.shape[-1],) #(kpls,timebls)
wgts_pI = wgts_pI.transpose([1,2,0]).copy() #(kpls,times,bltypes)
wgts_pI.shape = wgts_pI.shape[:-2] + (wgts_pI.shape[-2] * wgts_pI.shape[-1],) #(kpls,timebls)
...
for boot in xrange(NBOOT):
    ...
    for t in xrange(ntimes):
        ...
        dsum += [pk_2d[b,:,t] * wgts[:,t]]
        dwgt += [wgts[:,t]]
        pIsum += [pI_2d[b,:,t] * wgts_pI[:,t]]
        pIwgt += [wgts_pI[:,t]]
    if MEDIAN:
        dsum,dwgt = n.median(dsum, axis=0), n.median(dwgt, axis=0) #median over time
        pIsum,pIwgt = n.median(pIsum, axis=0), n.median(pIwgt, axis=0) #median over time
    else:
        dsum,dwgt = n.average(dsum,axis=0), n.average(dwgt,axis=0)
        pIsum,pIwgt = n.average(pIsum, axis=0), n.average(pIwgt, axis=0) #average over time
    pk_boot.append(dsum/dwgt)
    pI_boot.append(pIsum/pIwgt)

    dsum_fold = dsum[k0:].copy()
    dwgt_fold = dwgt[k0:].copy()
    pIsum_fold = pIsum[k0:].copy()
    pIwgt_fold = pIwgt[k0:].copy()
    dsum_fold[1:] = 0
    dwgt_fold[1:] = 0
    pIsum_fold[1:] = 0
    pIwgt_fold[1:] = 0
    ...
    pIsum_pos,pIwgt_pos = pIsum[k0+1:].copy(), pIwgt[k0+1:].copy()
    pIsum_neg,pIwgt_neg = pIsum[k0-1::-1].copy(), pIwgt[k0-1::-1].copy() #for odd # of channels    
    for h in xrange(2): #bootstrap over which half of the spectrum (or both) are used
        ...
        pIsum_fold[1:] += [pIsum_pos, pIsum_neg][h]
        pIwgt_fold[1:] += [pIwgt_pos, pIwgt_neg][h]

    pk_fold_boot.append(dsum_fold / dwgt_fold)
    pI_fold_boot.append(pIsum_fold / pIwgt_fold)

pk_boot = n.array(pk_boot).T
pk_fold_boot = n.array(pk_fold_boot).T
pI_boot = n.array(pI_boot).T
pI_fold_boot = n.array(pI_fold_boot).T
...
pI = n.average(pI_boot, axis=1) #average over all boots
pI_fold = n.average(pI_fold_boot, axis=1)
#this is excluding imag component in noise estimate `
pI_boot = n.sort(pI_boot.real, axis=1) #dropping imag component here
pI_fold_boot = n.sort(pI_fold_boot.real, axis=1) #dropping imag component here
...
if True:
    ...
    up_thresh = int(n.around(0.975 * pI_boot.shape[1])) #2 sigma, single tail
    #important to only include real component in estimation of error
    pI_err = (pI_boot[:,up_thresh] - pI.real) / 2 #effective "1 sigma" derived from actual 2 sigma
    up_thresh_fold = int(n.around(0.975 * pI_fold_boot.shape[1])) #2 sigma, single tail
    pI_err_fold = (pI_fold_boot[:,up_thresh_fold] - pI_fold.real) / 2 #effective "1 sigma" derived from actual 2 sigma

    print 'Plotting Pk...'
    print '%4s %11s %18s %18s %18s' % ('k','Pk+err','Pk +/- err', 'pI+err', 'pI +/- err')
    for k_,pk_,err_,pI_,pI_err_ in zip(kpl, pk.real/1e6, 2*err/1e6, pI.real/1e6, 2*pI_err/1e6):
        print '%6.3f, %9.3f (%9.3f +/- %9.3f), %9.3f (%9.3f +/- %9.3f)' % (k_, pk_+err_,pk_,err_,pI_+pI_err_,pI_, pI_err_)
    print '-'*70
    print 'Plotting Pk_fold...'
    print '%4s %15s %18s %18s %18s' % ('k','Pk_fold+err','Pk_fold +/- err', 'pI_fold+err', 'pI_fold +/- err' )
    for k_,pk_,err_,pI_,pI_err_ in zip(kpl[k0:], pk_fold.real/1e6, 2*err_fold/1e6, pI_fold.real/1e6, 2*pI_err_fold/1e6):
        print '%6.3f, %9.3f (%9.3f +/- %9.3f), %9.3f (%9.3f +/- %9.3f)' % (k_, pk_+err_,pk_,err_,pI_+pI_err_ , pI_, pI_err_)
else:
    err = n.std(pk_boot, axis=1)
    err_fold = n.std(pk_fold_boot, axis=1)
    pI_err = n.std(pI_boot, axis=1)
    pI_err_fold = n.std(pI_fold_boot, axis=1)
...
n.savez('pspec.npz', kpl=kpl, pk=pk, err=err,
        pk_fold=pk_fold, err_fold=err_fold,
        pI=pI, pI_err=pI_err, pI_fold=pI_fold, pI_err_fold=pI_err_fold,
        cmd=cmd, afreqs=afreqs,chans=chans,
