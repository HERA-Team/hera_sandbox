#!/usr/global/paper/CanopyVirtualEnvs/PAPER_CC/bin/python

### Adapted from plot_uv.py ###
...
o.add_option('--horizon', dest='horizon', default=None,
    help='Horizon limit in ns to plot as a vertical line if delay plot.')
    ...
    if is_chan_range and is_time_range:
        ...
        if opts.delay:
            if opts.chan_axis == 'index':
                ...
                plot_chans = delays
                xlabel = 'Delay (ns)'
                horizon = 1/(((3*1e8)/14.)/1e9) #XXX HERA 14-m horizon limit
                p.axvline(horizon,color='r')
                p.axvline(-horizon,color='r')
        else:
            if opts.chan_axis == 'index':
                plot_chans = chans
