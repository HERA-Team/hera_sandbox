...
PURPOSE:
      ...
      Used as a sanity check for vis_simulation.py
EXAMPLE CALL:
      ./vis_simulation_test.py 
AUTHOR:
      Carina Cheng
...
for ii, t in enumerate(times):
    ...
    for k in range(len(px)):
        ...
        sum = img[px[k][0]]+img[px[k][1]]+img[px[k][2]]+img[px[k][3]]
        if sum != 0:
            print k
            print img[px[k][0]]
            print img[px[k][1]]
    ...
    for jj, f in enumerate(freqs):
        ...
        fluxes = numpy.sum(img[px]*wgts, axis=1)

        #the 1 value is located in kex 3145724 out of 3145728 in img.map.map for 'pole' case

        toplot[ii] =  numpy.sum(fluxes)
        
        print numpy.where(fluxes != 0.)
