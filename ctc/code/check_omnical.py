...
o.add_option('-f', '--freqchan', default=0,help='Frequency channel to plot (for 1D plot). Default is 0.')
o.add_option('-t','--time',default=0,help='Time channel to plot (for 1D plot). Default is 0.')
...
### ChiSquared ###
    ...
    ymax = jds.max()-int(str(jds.min()).split('.')[0])
    ymin = jds.min()-int(str(jds.min()).split('.')[0])
    plt.imshow(cs,extent=(0,202,ymax,ymin),aspect='auto',interpolation='nearest')#,vmax=6)
    plt.xlabel('Freq Channel',fontsize=10)
