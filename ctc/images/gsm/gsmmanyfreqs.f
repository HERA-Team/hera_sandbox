! Angelica de Oliveira-Costa & Max Tegmark 2007
	...
	implicit none
	integer i,numfreqs
	real*8 startfreq,freqres,currentfreq
	character*60 outprefix,outfile,ComputeMatrixFilename3
	
	open(2,file='args.dat',status='old')
	read(2,*,end=777,err=777) outprefix,startfreq,freqres,numfreqs
	close(2)
	
	do i=1,numfreqs
		outfile=ComputeMatrixFilename3(1000+i,outprefix)
		currentfreq=startfreq+(i-1)*freqres
		call go(currentfreq,outfile)
	enddo
	stop
777	call usage
	end
	...
	subroutine usage
	print *, 'Runs GSM at a number of frequencies'
	print *, 'USAGE call gsmmanyfreqs.x GSM <Starting frequency in MHz> <Frequency resolution in MHz> <Number of Frequencies>'
	print *, 'EXAMPLE call gsmmanyfreqs.x GSM 12.0 1.0 100'
	return
	end		

	subroutine go(nu,outfile)
	implicit  none
	...
	print *,'Making sky map at frequency__________',nu
c	print *,'Outfile_____________________________',outfile(1:lnblnk(outfile))
...
c	print *,'Loading ',infile(1:lnblnk(infile))
...
1     IF (KHI-KLO.GT.1) THEN
	...
	character*60 function ComputeMatrixFilename3(perpband,prefix)
	! Returns name of compressed C,i file for i=band
	implicit none
	integer perpband,lnblnk
	character*60 prefix
	character*4 t
	
	call int2string(perpband,t)
	ComputeMatrixFilename3=prefix(1:lnblnk(prefix))//t//'.dat'
	
	return
	end
	
	! Subroutine for converting integers to strings
	subroutine int2string(num,str)
	implicit none
	character*4 str
	integer num
	write(str,'(i4.1)') num
