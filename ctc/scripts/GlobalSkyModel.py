...
class GlobalSkyModel:
    ...
    def __init__(self,freq,GSMlocation,GSMNSIDE):
        ...
        GSMComponents = np.loadtxt(GSMlocation + "component_maps_408locked.dat")
        components = np.loadtxt(GSMlocation + "components.dat")
        
        #temperature = np.exp(interp1d(np.log(components[:,0]), np.log(components[:,1]), kind='cubic')(np.log(freq))) #cubic spline interpolation in log(T)
        #weights = np.asarray([interp1d(np.log(components[:,0]), components[:,i+2], kind='cubic')(np.log(freq)) for i in range(3)]) #cubic spline interpolation for weights
        ...
        self.map = temperature*np.dot(GSMComponents,weights)
