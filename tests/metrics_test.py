...
class TestDataContainer(unittest.TestCase):
    ...
    def test_init(self):
        dc = capo.metrics.DataContainer(self.blpol)
        ...
        dc = capo.metrics.DataContainer(self.polbl)
        for k in dc._data.keys(): self.assertEqual(len(k), 3)
        self.assertEqual(set(self.bls), dc._bls)
        self.assertEqual(set(self.pols), dc._pols)
        dc = capo.metrics.DataContainer(self.both)
        for k in dc._data.keys(): self.assertEqual(len(k), 3)
        self.assertEqual(set(self.bls), dc._bls)
        self.assertEqual(set(self.pols), dc._pols)
        self.assertRaises(AssertionError, capo.metrics.DataContainer, {(1,2,3,4):2})
    def test_bls(self):
        dc = capo.metrics.DataContainer(self.blpol)
...
class TestMethods(unittest.TestCase):
    ...
    def test_check_ants(self):
        ...
        cnts = capo.metrics.check_ants(reds, data)
        ...
        cnts = capo.metrics.check_ants(reds, data, flag_thresh=.3)
        for i in [1,2,3,4,5]: self.assertEqual(cnts[(i,'xx')], 0)
        for bl in data['xx']:
    ...
    def test_check_noise_variance(self):
        ...
        nos = capo.metrics.check_noise_variance(data, wgts, 1., 1.)
        for bl in data['xx']:
            n = nos[bl+('xx',)]
            self.assertEqual(n.shape, (100-1,))
            np.testing.assert_almost_equal(n, np.ones_like(n), 0)
        nos = capo.metrics.check_noise_variance(data, wgts, 1., 10.)
        for bl in data['xx']:
            n = nos[bl+('xx',)]
            self.assertEqual(n.shape, (100-1,))
            np.testing.assert_almost_equal(n, 10*np.ones_like(n), -1)
        nos = capo.metrics.check_noise_variance(data, wgts, 10., 10.)
        for bl in data['xx']:
            n = nos[bl+('xx',)]
            self.assertEqual(n.shape, (100-1,))
