...
for basefile in filegroups.keys():
    for filename in filegroups[basefile]:
        f = n.load(filename)
        if filename.find('times') != -1: times = f['times']
        elif filename.find('afreqs') != -1: afreqs = f['freqs']
        elif filename.find('srcdata') != -1:
            #src = filename.split('.')[-2].split('_')[-1]
            src = filename.split('srcdata_')[-1][:-len('.npz')]
            srcdata[src] = {}
            for bl in f.files: srcdata[src][int(bl)] = f[bl]
...
for i, src in enumerate(srckeys):
    ...
    srcdata[src] = sum([srcdata[src][bl] for bl in srcdata[src]]) / nbls
    srcdata[src] = n.array(srcdata[src]).real.clip(.01,n.Inf)
    d,t = srcdata[src], times
    order = n.argsort(t)
