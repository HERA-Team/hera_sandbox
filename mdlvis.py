...
a.scripting.add_standard_options(o, cal=True, src=True,ant=True)
...
log = logging.getLogger('pgb015_v005_nc_ion')
...
def mdl(uv, p, d, f):
    ...
    if curtime != t:
        ...
        cat.compute(aa)
        P = cat.get_params()
        P.update(aa.get_params())
        tracelog(uvofile+'/',P,t,aa.sidereal_time())        
...
if len(args) > 0:
    ...
    lsts = []
    if not opts.prnt is None: print "#", '\t'.join(opts.prnt.split(','))
    for cnt,t in enumerate(times):
        if opts.prnt is None: print '#Timestep %d / %d' % (cnt+1, len(times))
        else: print t
        aa.set_jultime(t)
    ...
    del(uv)
    print "writing model trace for the following parameters in",uvofile
    
#    print "#jd\tlst\t","\t".join(cat.tracelines)
#    trace = n.array([(t,lst)+c for t,lst,c in zip(times,lsts,cat.trace)],
#        dtype={'names':('jd','lst')+cat.tracelines,'formats':(n.float,n.float,)+cat.tracetypes})
#    cattrace = tab.tabarray(array=trace,dtype=trace.dtype)
#    cattrace.saveSV('new.txt',comments='#')
